...
import { listRequesterSupportCases } from "@/lib/admin/support";
...
export default async function SupportPage() {
  ...
  const [cases, memberships] = await Promise.all([
    listRequesterSupportCases(database, {
      userId: user.id,
      status: user.status,
    }),
  ...
  return (
    <main className="page-shell py-8 sm:py-12">
      ...
      <div className="mt-6 grid gap-8 lg:grid-cols-[minmax(0,1fr)_24rem]">
        <section>
          <p className="eyebrow">Authenticated Support</p>
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Operatives Ziel {item.dueAt.toLocaleString("de-CH")}
                    </p>
                  </div>
        ...
        <aside>
          <h2 className="mb-3 text-lg font-semibold">Neue Anfrage</h2>
          <SupportRequestForm
