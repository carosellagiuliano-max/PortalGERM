...
import { SupportReplyForm } from "@/components/admin/support-request-form";
...
import { getRequesterSupportCase } from "@/lib/admin/support";
...
export default async function SupportCasePage({
  ...
  const item = await getRequesterSupportCase(
    getDatabase(),
    ...
    <main className="page-shell max-w-4xl py-8 sm:py-12">
      ...
      <header className="mt-6">
        ...
        <p className="mt-2 text-muted-foreground">
          {item.category} · operatives Ziel {formatDateTime(item.dueAt)}
        </p>
      </header>
      ...
      <ol className="mt-6 grid gap-3">
        {item.events.map((event, index) => (
          <li key={`${event.kind}-${index}`} className="rounded-lg border p-4">
      ...
      {item.status === "WAITING_FOR_REQUESTER" ? (
        <div className="mt-6">
          <SupportReplyForm caseId={item.id} idempotencyKey={randomUUID()} />
        </div>
      ) : null}
    </main>
  );
