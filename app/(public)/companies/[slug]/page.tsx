...
const getCompany = cache((slug: string) =>
  getPublicCompanyDetailBySlug(slug, listPublicJobsForCompany),
);
...
export async function generateMetadata({
  ...
  const description =
    ...
    `${company.name}: öffentliches Unternehmensprofil und aktuelle Stellen.`;
  const indexable =
...
export default async function CompanyDetailPage({ params }: CompanyPageProps) {
  ...
  return (
    <div className="page-shell py-10 sm:py-14">
      ...
      >
        <ArrowLeftIcon className="size-4" aria-hidden="true" /> Zu den
        Unternehmen
      </Link>
      <section className="mt-6 grid gap-8 lg:grid-cols-[minmax(0,1fr)_20rem] lg:items-start">
        <div>
          ...
          <div className="mt-5 flex flex-wrap items-center gap-3">
            ...
            {company.trust ? (
              <CompanyTrustBadge trust={company.trust} showDetails />
            ) : null}
          </div>
          <div className="mt-4 flex flex-wrap gap-x-5 gap-y-2 text-muted-foreground">
            ...
            {company.city === null && company.canton === null ? null : (
              <span className="inline-flex items-center gap-1.5">
                <MapPinIcon className="size-4" aria-hidden="true" />
                {[company.city, company.canton].filter(Boolean).join(", ")}
              </span>
            )}
            {company.size === null ? null : (
              <span>{company.size} Mitarbeitende</span>
            )}
          </div>
        </div>
        <aside className="rounded-xl border bg-card p-5 shadow-sm">
          ...
          <p className="mt-2 text-sm leading-6 text-muted-foreground">
            Starte einen kontrollierten Firmenanspruch. Der Einstieg verleiht
            weder Eigentum noch Zugriff.
          </p>
          {claimHref !== null ? (
            <Link
              href={claimHref}
            ...
            >
              <ShieldQuestionIcon aria-hidden="true" /> Firma beanspruchen
            </Link>
            ...
            <p className="mt-4 rounded-lg bg-muted p-3 text-sm text-muted-foreground">
              Ein Firmenanspruch benötigt einen separaten Arbeitgeberzugang.
            </p>
          )}
          {company.website === null ? null : (
            <a
            ...
            >
              Website besuchen <ExternalLinkIcon data-icon="inline-end" />
            </a>
          )}
        </aside>
      </section>
      ...
      <div className="mt-10 grid gap-8 lg:grid-cols-[minmax(0,1fr)_20rem] lg:items-start">
        <article className="grid gap-8">
          <section>
            <h2 className="text-2xl font-semibold">Über {company.name}</h2>
            <p className="mt-4 whitespace-pre-line leading-7 text-muted-foreground">
              {company.about ??
                "Dieses Unternehmen hat noch keine öffentliche Beschreibung hinterlegt."}
            </p>
          </section>
          {company.enhancedProfile && company.values.length > 0 ? (
            <TextList title="Werte" values={company.values} />
          ) : null}
          {company.enhancedProfile && company.benefits.length > 0 ? (
            <TextList title="Benefits" values={company.benefits} />
          ) : null}
        </article>
      ...
      <section
        className="mt-14 border-t pt-12"
        aria-labelledby="company-jobs-title"
      >
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <p className="eyebrow">Aktuelle Stellen</p>
            <h2 id="company-jobs-title" className="mt-2 text-3xl font-semibold">
              Jobs bei {company.name}
            </h2>
          </div>
          <Badge variant="outline">{company.openJobCount} offen</Badge>
        </div>
        <div className="mt-7">
          <JobGrid
            jobs={company.jobs}
            emptyText="Dieses Unternehmen hat aktuell keine öffentlich verfügbaren Stellen."
          />
        </div>
      </section>
    </div>
    ...
    <section>
      ...
      <ul className="mt-4 grid gap-3 sm:grid-cols-2">
        {values.map((value, index) => (
          <li
            key={`${index}-${value}`}
            className="rounded-lg border bg-card p-4 leading-6 text-muted-foreground"
          >
            {value}
          </li>
        ))}
