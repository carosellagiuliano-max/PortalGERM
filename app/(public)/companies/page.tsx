...
const COMPANIES_METADATA = Object.freeze({
  title: "Unternehmen",
  description:
    "Aktive öffentliche Unternehmensprofile und ihre aktuellen Stellen entdecken.",
});
  ...
  return (
    <div className="page-shell py-12 sm:py-16">
      ...
      <h1 className="mt-3 text-balance text-4xl font-semibold tracking-tight sm:text-5xl">
        Lerne Arbeitgeber kennen, bevor du dich bewirbst.
      </h1>
      <p className="mt-4 max-w-3xl text-lg leading-8 text-muted-foreground">
        Die Sichtbarkeit eines Profils folgt dem aktiven Firmenstatus. Eine
        Verifikation wird separat und transparent als Badge ausgewiesen.
      </p>
      <form
        action="/companies"
        method="get"
        className="mt-8 grid gap-4 rounded-xl border bg-card p-5 shadow-sm sm:grid-cols-2 lg:grid-cols-[2fr_1fr_1fr_auto] lg:items-end"
      >
        ...
        <label className="grid gap-1.5 text-sm font-medium">
          ...
          <select
            name="canton"
            defaultValue={cantonSlug ?? ""}
            className="h-10 rounded-lg border border-input bg-background px-3 text-sm"
          >
            <option value="">Alle Kantone</option>
            {catalog.cantons.map((canton) => (
              <option key={canton.id} value={canton.slug}>
                {canton.name}
              </option>
            ))}
          </select>
      ...
      <div className="mt-10">
        <p className="text-sm text-muted-foreground">Öffentliche Profile</p>
        <h2 className="mt-1 text-2xl font-semibold">
          {directory.totalEligible} Unternehmen
        </h2>
      </div>
      {directory.invalidCursor ? (
        ...
        >
          Der Seitenzeiger war ungültig. Wir zeigen dir die erste Seite.
        </p>
      ) : null}
      {directory.companies.length === 0 ? (
        <div className="mt-6 rounded-xl border border-dashed bg-muted/25 p-10 text-center text-muted-foreground">
          Keine passenden öffentlichen Unternehmen gefunden. Passe die Filter
          an.
        </div>
      ) : (
      ...
      {directory.nextCursor === null ? null : (
        <div className="mt-8 flex justify-center">
          <Link
            href={companyDirectoryHref({
              query,
              cantonSlug,
              industry,
              verifiedOnly,
...
function companyDirectoryHref(
  ...
  if (input.cantonSlug) params.set("canton", input.cantonSlug);
  if (input.industry) params.set("industry", input.industry);
  if (input.verifiedOnly) params.set("verified", "true");
