...
export default async function ClusterAssessmentPage({ params }: Readonly<{ params: Promise<{ id: string }> }>) {
  ...
  const dependencies = { actor: { userId: admin.id, email: admin.email, role: admin.role, status: admin.status, capabilities: admin.capabilities }, correlationId: "admin-cluster-detail", database: getDatabase(), now: new Date() } as const;
