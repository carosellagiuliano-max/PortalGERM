...
import { reviewAdminSearchLearning } from "@/lib/admin/search-learning";
import {
  triageAbuseReport,
...
import {
  ...
  schedulePlanVersion,
  scheduleProductVersion,
} from "@/lib/billing/admin-billing";
import {
  adminMockRenewSubscription,
  type AdminMockRenewalResult,
} from "@/lib/billing/admin-renewal";
import {
  cancelAdminOrder,
  voidAdminInvoice,
...
import {
  ...
  type SubscriptionBoundaryProjectionResult,
} from "@/lib/billing/subscriptions";
import {
  projectDueCreditExpiries,
  type CreditExpiryProjectionResult,
} from "@/lib/billing/credits";

export async function adminCommandAction(
  ...
  if (operation === null)
    return errorState("Das Formular ist unvollständig.", "INVALID_INPUT");
  ...
  try {
    const result =
      operation === "job-start-review"
        ? await startAdminJobReview(input as never, dependencies)
        : operation === "job-request-changes"
          ? await requestAdminJobChanges(input as never, dependencies)
          : operation === "job-approve"
            ? await approveAdminJob(input as never, dependencies, emailProvider)
            : operation === "job-reject"
              ? await rejectAdminJob(
                  input as never,
                  dependencies,
                  emailProvider,
                )
              : operation === "job-publish"
                ? await publishAdminJob(input as never, dependencies)
                : operation === "job-boost-cancel"
                  ? await cancelAdminJobBoost(input, dependencies)
                  : operation === "boost-status-project"
                    ? await projectAdminBoostStatuses(input, dependencies)
                    : operation === "verification-evidence"
                      ...
                      : operation === "verification-verify"
                        ? await verifyCompany(input as never, dependencies)
                                  ...
                                  : operation === "claim-reject"
                                    ...
                                    : operation === "claim-approve"
                                      ...
                                      : operation === "user-suspend"
                                        ? await suspendUser(input, dependencies)
                                        : operation === "user-reactivate"
                                          ? await reactivateUser(
                                              input,
                                              dependencies,
                                            )
                                          : operation === "user-force-logout"
                                            ? await forceLogoutUser(
                                                input,
                                                dependencies,
                                              )
                                            : operation === "taxonomy-mutate"
                                              ? await mutateAdminTaxonomy(
                                                  input,
                                                  dependencies,
                                                )
                                              : operation ===
                                                  "search-learning-review"
                                                ? await reviewAdminSearchLearning(
                                                    input,
                                                    dependencies,
                                                  )
                                                : operation === "report-triage"
                                                  ...
                                                  : operation ===
                                                    ...
                                                    : operation ===
                                                        "restriction-lift"
                                                      ? await liftModerationRestriction(
                                                          input,
                                                          dependencies,
                                                        )
                                                      : operation ===
                                                          "restriction-expire"
                                                        ? await expireModerationRestriction(
                                                            input,
                                                            dependencies,
                                                          )
                                                        : operation ===
                                                                  ...
                                                                  : operation ===
                                                                    ...
                                                                    : operation ===
                                                                      ...
                                                                      : operation ===
                                                                        ...
                                                                        : operation ===
                                                                            "import-setup-expire"
                                                                          ? await expireImportSetup(
                                                                              input,
                                                                              dependencies,
                                                                            )
                                                                          : operation ===
                                                                              "support-manage"
                                                                            ? await manageSupportCase(
                                                                                input,
                                                                                dependencies,
                                                                              )
                                                                            : operation ===
                                                                              ...
                                                                              : operation ===
                                                                                ...
                                                                                : operation ===
                                                                                    "cluster-evaluate"
                                                                                  ? await evaluateClusterLaunch(
                                                                                      input,
                                                                                      dependencies,
                                                                                    )
                                                                                  : operation ===
                                                                                      "cluster-expire"
                                                                                    ? await projectExpiredClusterLaunches(
                                                                                        input,
                                                                                        dependencies,
                                                                                      )
                                                                                    : operation ===
                                                                                      ...
                                                                                      : operation ===
                                                                                          "lead-manage"
                                                                                        ? await manageSalesLead(
                                                                                            input,
                                                                                            dependencies,
                                                                                          )
                                                                                        : operation ===
                                                                                            "sla-project"
                                                                                          ? await projectAdminSlaAlerts(
                                                                                              input as never,
                                                                                              dependencies,
                                                                                            )
                                                                                          : operation ===
                                                                                              "system-task-outcome"
                                                                                            ? await recordSystemTaskOutcome(
                                                                                                input,
                                                                                                dependencies,
                                                                                              )
                                                                                            : operation ===
                                                                                                "tax-rate-approve"
                                                                                              ? await approveTaxRateVersion(
                                                                                                  input,
                                                                                                  dependencies,
                                                                                                )
                                                                                              : operation ===
                                                                                                ...
                                                                                                : operation ===
                                                                                                  ...
                                                                                                  : operation ===
                                                                                                    ...
                                                                                                    : operation ===
                                                                                                        "invoice-void"
                                                                                                      ? await voidAdminInvoice(
                                                                                                          input,
                                                                                                          dependencies,
                                                                                                        )
                                                                                                      : operation ===
                                                                                                          "catalog-plan-schedule"
                                                                                                        ? await schedulePlanVersion(
                                                                                                            input,
                                                                                                            dependencies,
                                                                                                          )
                                                                                                        : operation ===
                                                                                                            "catalog-product-release-decide"
                                                                                                          ? await recordProductReleaseDecision(
                                                                                                              input,
                                                                                                              dependencies,
                                                                                                            )
                                                                                                          : operation ===
                                                                                                              "catalog-product-schedule"
                                                                                                            ? await scheduleProductVersion(
                                                                                                                input,
                                                                                                                dependencies,
                                                                                                              )
                                                                                                            : operation ===
                                                                                                                "catalog-plan-deactivate"
                                                                                                              ? await deactivatePlanVersion(
                                                                                                                  input,
                                                                                                                  dependencies,
                                                                                                                )
                                                                                                              : operation ===
                                                                                                                  "catalog-product-deactivate"
                                                                                                                ? await deactivateProductVersion(
                                                                                                                    input,
                                                                                                                    dependencies,
                                                                                                                  )
                                                                                                                : operation ===
                                                                                                                    "catalog-project-due"
                                                                                                                  ? await projectDueCatalogVersions(
                                                                                                                      input,
                                                                                                                      dependencies,
                                                                                                                    )
                                                                                                                  : operation ===
                                                                                                                      "subscription-renew-mock"
                                                                                                                    ? await adminMockRenewSubscription(
                                                                                                                        input,
                                                                                                                        dependencies,
                                                                                                                      )
                                                                                                                    : operation ===
                                                                                                                        "subscription-boundaries-project"
                                                                                                                      ? await projectDueSubscriptionBoundaries(
                                                                                                                          input,
                                                                                                                          dependencies,
                                                                                                                        )
                                                                                                                      : operation ===
                                                                                                                          "credit-expiries-project"
                                                                                                                        ? await projectDueCreditExpiries(
                                                                                                                            input,
                                                                                                                            dependencies,
                                                                                                                          )
                                                                                                                        : null;
    if (result === null)
      return errorState("Unbekannte Admin-Aktion.", "INVALID_INPUT");
    ...
    if (operation === "subscription-boundaries-project") {
      ...
      return Object.freeze({
        status: "success",
        message: subscriptionProjectionMessage(
          result.value as SubscriptionBoundaryProjectionResult,
        ),
      });
    }
    ...
    return Object.freeze({
      status: "success",
      message: result.replay
        ? "Aktion war bereits sicher verarbeitet."
        : "Aktion wurde sicher verarbeitet.",
    });
  } catch {
    return errorState(
      "Die Aktion konnte nicht vollständig ausgeführt werden.",
      "WRITE_FAILED",
    );
  }
}
...
function singleValue(formData: FormData, key: string) {
  const values = formData.getAll(key);
  return values.length === 1 && typeof values[0] === "string"
    ? values[0].trim()
    : null;
}
function errorState(message: string, code: string): AdminActionState {
  return Object.freeze({ status: "error", message, code });
}
function messageForCode(code: string) {
  return code === "CONFLICT"
    ? "Der Datensatz wurde inzwischen geändert. Bitte lade den aktuellen Stand neu."
    : code === "FORBIDDEN"
      ? "Für diese Aktion fehlt die Berechtigung."
      : code === "NOT_FOUND"
        ? "Der Datensatz ist nicht mehr verfügbar."
        : code === "QUOTA_EXCEEDED"
          ? "Das aktuelle Nutzungslimit verhindert diese Aktion."
          : code === "DUPLICATE"
            ? "Ein inhaltlich identisches Inserat dieser Firma ist bereits aktiv. Die Veröffentlichung wurde verhindert."
            : code === "RESTRICTED"
              ? "Eine aktive Moderationssperre verhindert diese Aktion."
...
function revalidateSubscriptionBoundaryPaths() {
  for (const path of ["/employer/billing", "/employer/jobs", "/employer/team"])
    revalidatePath(path);
}
function subscriptionProjectionMessage(
  result: SubscriptionBoundaryProjectionResult,
) {
  return `Projektion abgeschlossen: ${result.appliedCancellationCount} Kündigung(en), ${result.appliedDowngradeCount} Downgrade(s), ${result.expiredSubscriptionCount} natürliche Vertragsabläufe angewendet.`;
}
