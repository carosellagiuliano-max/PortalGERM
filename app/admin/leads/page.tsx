...
export default async function AdminLeadsPage({ searchParams }: Readonly<{ searchParams: Promise<{ status?: string; owner?: string; overdue?: string }> }>) {
  ...
  return <div className="grid gap-6"><header><p className="eyebrow">Sales Operations</p><h1 className="mt-2 text-3xl font-semibold">Leads</h1><p className="mt-2 text-muted-foreground">Neue und zugewiesene Leads werden nach dem 24-Stunden-Ziel geordnet; Überfälligkeit ist sichtbar.</p></header><form method="get" className="grid gap-2 rounded-lg border bg-card p-3 sm:grid-cols-4"><label className="grid gap-1 text-xs font-medium text-muted-foreground">Status<select name="status" defaultValue={query.status ?? ""} className="h-9 rounded-lg border px-3 text-sm text-foreground"><option value="">Alle Status</option><option>NEW</option><option>CONTACTED</option><option>QUALIFIED</option><option>WON</option><option>LOST</option></select></label><label className="grid gap-1 text-xs font-medium text-muted-foreground">Owner<select name="owner" defaultValue={query.owner ?? ""} className="h-9 rounded-lg border px-3 text-sm text-foreground"><option value="">Alle Owner</option><option value="UNASSIGNED">Nicht zugewiesen</option>{owners.map((owner) => <option key={owner.id} value={owner.id}>{owner.name ?? owner.email}</option>)}</select></label><label className="flex items-end gap-2 pb-2 text-sm"><input type="checkbox" name="overdue" value="true" defaultChecked={query.overdue === "true"} /> nur überfällig</label><button type="submit" className="self-end rounded-lg bg-primary px-3 py-2 text-sm text-primary-foreground">Filtern</button></form><LeadList leads={leads} now={now} /></div>;
}
