...
export async function sendInvitationAction(_state: EmployerActionState, formData: FormData): Promise<EmployerActionState> {
  ...
  const result = await sendCompanyInvitation(deps.companyId, deps.actor, {
    email: formData.get("email"),
    role: formData.get("role"),
    ...stepUpFields(formData),
  }, deps);
...
export async function changeMemberRoleAction(_state: EmployerActionState, formData: FormData): Promise<EmployerActionState> {
  const deps = await dependencies();
  if (deps === null) return error("Die Anfrage konnte nicht sicher bestätigt werden.");
  const result = await changeCompanyMemberRole(deps.companyId, deps.actor, {
...
export async function removeMemberAction(_state: EmployerActionState, formData: FormData): Promise<EmployerActionState> {
  const deps = await dependencies();
  if (deps === null) return error("Die Anfrage konnte nicht sicher bestätigt werden.");
  const result = await removeCompanyMember(deps.companyId, deps.actor, {
    membershipId: formData.get("membershipId"),
...
async function dependencies() {
  const [context, request, pageUser] = await Promise.all([
    getEmployerContext(),
  ...
  if (
    ...
    (context.current.membershipRole !== "OWNER" &&
      context.current.membershipRole !== "ADMIN") ||
    (pageUser.role !== "EMPLOYER" && pageUser.role !== "RECRUITER")
  ) {
    return null;
  }
  return {
    ...
    emailProvider,
    stepUpActor: {
      sessionId: pageUser.sessionId,
      globalRole: pageUser.role,
    },
  } as const;
}
