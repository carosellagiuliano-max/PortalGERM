...
import {
  PHASE_12_BILLING_ADMIN_CAPABILITIES,
  hasAdminCapability,
} from "@/lib/admin/capabilities";
import {
  catalogRangesOverlap,
  grantAdminCredits,
  listAdminOrders,
...
describe("Phase 12 Admin Billing authorization boundary", () => {
  it("names read, catalog, analytics, grant and exact reversal capabilities", () => {
    ...
    for (const capability of PHASE_12_BILLING_ADMIN_CAPABILITIES) {
      ...
      expect(
        hasAdminCapability({ ...actor, role: "EMPLOYER" }, capability),
      ).toBe(false);
      expect(
        hasAdminCapability({ ...actor, status: "SUSPENDED" }, capability),
      ).toBe(false);
    }
  });
  ...
  it("fails closed before any database read for a non-admin", async () => {
    const dependencies = {
      actor: { ...actor, role: "EMPLOYER" },
      correlationId: "22000000-0000-4000-8000-000000000001",
    ...
    await expect(
      reverseCreditConsume(
        {
          entryId: "55000000-0000-4000-8000-000000000001",
    ...
    await expect(projectDueCatalogVersions({}, dependencies)).resolves.toEqual({
      ok: false,
      code: "FORBIDDEN",
    });
    await expect(
      projectDueSubscriptionBoundaries({}, dependencies),
    ).resolves.toEqual({ ok: false, code: "FORBIDDEN" });
  });
  ...
  it("accepts no client-controlled subscription projector scope or clock", async () => {
    ...
    await expect(
      projectDueSubscriptionBoundaries(
        { scheduleId: "33000000-0000-4000-8000-000000000001" },
        dependencies,
      ),
    ).resolves.toEqual({ ok: false, code: "INVALID_INPUT" });
  });
  ...
  it("exposes only working Phase 12 admin navigation destinations", () => {
    ...
    for (const route of ["billing", "plans", "products", "analytics"])
      expect(
        ADMIN_NAVIGATION.some(({ href }) => href === `/admin/${route}`),
      ).toBe(true);
  });
  ...
  it("exposes the same server-clock projector trigger on both catalog pages", () => {
    ...
    const plans = readFileSync(
      join(root, "app", "admin", "plans", "page.tsx"),
      "utf8",
    );
    const products = readFileSync(
      join(root, "app", "admin", "products", "page.tsx"),
      "utf8",
    );
    expect(actions).toMatch(/operation\s*===\s*"catalog-project-due"/u);
  ...
  it("exposes a server-clock subscription boundary trigger on the Billing page", () => {
    ...
    const billing = readFileSync(
      join(root, "app", "admin", "billing", "page.tsx"),
      "utf8",
    );
  ...
  it("uses half-open catalog ranges so exact successor boundaries are allowed", () => {
    ...
    expect(
      catalogRangesOverlap(start, end, new Date(boundary.getTime() - 1), null),
    ).toBe(true);
  });
});
