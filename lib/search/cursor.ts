...
const organicTupleSchema = z
  .discriminatedUnion("sort", [
    z
      .object({
...
const cursorPayloadSchema = z
  .object({
    ...
    searchPolicyVersion: z.literal(
      SEARCH_POLICY_RELEASE_V2.searchPolicyVersion,
    ),
    taxonomyVersion: z.literal(SEARCH_POLICY_RELEASE_V2.taxonomyVersion),
    rankingVersion: z.literal(SEARCH_POLICY_RELEASE_V2.rankingVersion),
    queryHash: z.string().regex(/^[a-f0-9]{64}$/),
    rankingAsOf: z.iso.datetime({ offset: true }),
    responseProjectionFingerprint: z
      .string()
      .regex(/^[a-f0-9]{32}$/)
      .optional(),
...
function signature(encoded: string, secret: string): Buffer {
  return createHmac("sha256", secret)
    .update(`search-cursor-v1\0${encoded}`)
    .digest();
}
...
export function encodeSearchCursor(
  ...
  if (secret.length < 32) {
    throw new TypeError(
      "Cursor signing key must contain at least 32 characters.",
    );
  }
  const parsed = cursorPayloadSchema.parse(payload);
  const encoded = Buffer.from(JSON.stringify(parsed), "utf8").toString(
    "base64url",
  );
...
export function decodeSearchCursor(
  ...
  try {
    ...
    if (!result.success || result.data.queryHash !== expected.queryHash)
      return null;
    if (
      result.data.organicTuple !== null &&
