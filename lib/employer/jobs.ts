...
import {
  deactivatePublicationFingerprint,
...
export const jobWizardStepOneSchema = z
  .strictObject({
    ...
    if (value.remoteType === "REMOTE") {
      if (value.remoteCountryCode !== "CH") {
        context.addIssue({
          ...
          message: "REMOTE_COUNTRY_MUST_BE_CH",
        });
      }
      if (value.cantonId !== null || value.cityId !== null) {
        context.addIssue({
          code: "custom",
          path: ["cityId"],
          message: "REMOTE_LOCATION_MUST_NOT_HAVE_CITY",
        });
      }
    } else {
      if (value.remoteCountryCode !== null) {
        context.addIssue({
          code: "custom",
          path: ["remoteCountryCode"],
          message: "LOCAL_LOCATION_MUST_NOT_HAVE_COUNTRY",
        });
      }
      if (value.cantonId === null || value.cityId === null) {
        context.addIssue({
          code: "custom",
          path: ["cityId"],
          message: "LOCAL_LOCATION_REQUIRES_CITY",
        });
      }
    }
    if (
      new Set(value.languages.map(({ code }) => code)).size !==
...
export const jobWizardStepTwoSchema = z
  .strictObject({
  ...
  .superRefine((value, context) => {
    ...
    if (
      new Set(value.benefits.map(({ benefitCode }) => benefitCode)).size !==
...
export const jobWizardStepThreeSchema = z
  .strictObject({
    ...
    requiredDocumentKinds: z
      .array(z.enum(REQUIRED_DOCUMENT_KINDS))
      .min(1)
      .max(7),
  ...
  .superRefine((value, context) => {
    ...
    if (
      salaryParts.some((part) => part !== null) &&
      salaryParts.some((part) => part === null)
    ) {
      context.addIssue({
        code: "custom",
        path: ["salaryPeriod"],
        message: "SALARY_ALL_OR_NONE",
      });
    }
    if (
      ...
      context.addIssue({
        code: "custom",
        path: ["salaryMax"],
        message: "INVALID_SALARY_RANGE",
      });
    }
    if (
      new Set(value.requiredDocumentKinds).size !==
      value.requiredDocumentKinds.length
    ) {
      context.addIssue({
        code: "custom",
    ...
    if (
      value.requiredDocumentKinds.includes("NONE") &&
      value.requiredDocumentKinds.length !== 1
    ) {
      context.addIssue({
        code: "custom",
        path: ["requiredDocumentKinds"],
        message: "NONE_MUST_BE_EXCLUSIVE",
      });
    }
    if (
      !isValidApplicationContact(
        value.applicationContactKind,
        value.applicationContactValue,
      )
    ) {
      context.addIssue({
        code: "custom",
...
export function buildEmployerJobScopeWhere(
  ...
  const assignmentScope =
    actor.membershipRole === "RECRUITER"
      ? {
          assignments: {
            some: {
              companyId: actor.companyId,
  ...
  return {
    ...
    company: {
      status:
        options.activeCompanyOnly === true
          ? "ACTIVE"
          : { in: ["DRAFT", "ACTIVE"] },
    ...
    rows.map((row) => {
      ...
      return Object.freeze({
        ...
        location:
          ...
          revision?.city?.name ??
          revision?.canton?.name ??
          "Remote / offen",
        applications: row._count.applications,
        views: row.viewAggregates.reduce(
          (sum, aggregate) => sum + aggregate.viewCount,
          0,
        ),
        ...
        capabilities: resolveEmployerJobCapabilities(
...
export type EmployerJobDetail =
  EmployerJobSummaryDetail | EmployerJobFullDetail;
...
export async function getEmployerJobDetail(
  ...
  const assignmentRole =
    actor.membershipRole === "RECRUITER"
      ? (summary.assignments[0]?.role ?? null)
      : null;
  const capabilities = resolveEmployerJobCapabilities(
    actor.membershipRole,
    assignmentRole,
  );
  if (!capabilities.readFullRevision) {
    return Object.freeze({
      ...
      location:
        summary.currentRevision?.locationLabel ??
        summary.currentRevision?.city?.name ??
        summary.currentRevision?.canton?.name ??
        "Remote / offen",
      applications: summary._count.applications,
      views: summary.viewAggregates.reduce(
  ...
  const fullAssignmentRoles =
    actor.membershipRole === "RECRUITER"
      ? (["EDITOR", "REVIEWER"] as const)
      : undefined;
  const [job, auditEvents, freshness] = await Promise.all([
    database.job.findFirst({
      where: {
        id: jobId,
        ...buildEmployerJobScopeWhere(actor, now, {
          assignmentRoles: fullAssignmentRoles,
        }),
        status: { not: "REMOVED" },
      },
      ...
      where: {
        ...
        OR: [
          ...
          ...(summary.currentRevision === null
            ...
            : [
                {
                  targetType: "JOB_REVISION" as const,
                  targetId: summary.currentRevision.id,
                },
              ]),
        ],
      },
    ...
    database.jobFreshnessProjection.findUnique({
      where: { jobId },
      select: {
        state: true,
        dueAt: true,
  ...
  const score =
    revision === null
      ...
      : calculateFairJobScoreV2(
          buildFairJobInputV2({
            job: { id: job.id },
            revision: toFairJobRevision(revision),
          }),
          { now },
        );
  ...
  return Object.freeze({
    ...
    latestScoreSnapshot:
      scoreSnapshot === undefined
        ? null
        : Object.freeze({
            points: scoreSnapshot.scorePoints,
            maxPoints: scoreSnapshot.maxPoints,
            calculatedAt: scoreSnapshot.calculatedAt,
          }),
    freshness: freshness === null ? null : Object.freeze(freshness),
...
export const INITIAL_EMPLOYER_JOB_FORM_STATE: EmployerJobFormState =
...
export async function createEmployerJobDraft(
  ...
  if (!parsed.success || !key.success)
    return failure("INVALID_INPUT", zodIssues(parsed));
  const now = validNow(dependencies.now);
  const actor = dependencies.actor;
  if (actor.membershipRole === "VIEWER") return failure("FORBIDDEN");
  if (
    parsed.data.validThrough !== null &&
    !isFuturePublicationDate(parsed.data.validThrough, now)
  ) {
  ...
  try {
    return await dependencies.database.$transaction(
      async (transaction) => {
        ...
        if (
          replay?.job.currentRevision !== null &&
          replay?.job.currentRevision !== undefined
        ) {
          return success(
            {
              jobId: replay.job.id,
        ...
        const catalogValid = await validateStepOneCatalog(
          transaction,
          parsed.data,
        );
        ...
        const validThrough =
          parsed.data.validThrough ?? new Date(now.getTime() + 30 * DAY);
        const initialRevision = initialRevisionSnapshot(
          jobId,
          revisionId,
          revisionNumber,
          parsed.data,
      ...
      async (transaction) => {
        const replay = await transaction.jobStatusEvent.findFirst({
          where: {
            ...
            job: buildEmployerJobScopeWhere(actor, now, {
              assignmentRoles:
                actor.membershipRole === "RECRUITER" ? ["EDITOR"] : undefined,
              activeCompanyOnly: true,
            }),
          },
          select: {
        ...
        const source = await loadLockedMutationJob(
          transaction,
          actor,
          parsed.data.jobId,
          now,
        ...
        const sourceCapabilities = resolveEmployerJobCapabilities(
          actor.membershipRole,
          source.assignments[0]?.role ?? null,
        );
        if (
          !sourceCapabilities.manageLifecycle &&
          !sourceCapabilities.mutateDraft
        )
          return failure("NOT_FOUND");
        if (
          source.version !== parsed.data.expectedJobVersion ||
          source.currentRevision.version !== parsed.data.expectedRevisionVersion
        ) {
        ...
        const jobId = randomUUID();
        const revisionId = randomUUID();
        const validThrough = isFuturePublicationDate(
...
export type SaveEmployerJobStepInput = JobCommandEnvelope &
  Readonly<{
    step: 1 | 2 | 3;
...
export async function saveEmployerJobStep(
  ...
  if (!envelope.success || !dataResult.success)
    return failure("INVALID_INPUT", zodIssues(dataResult));
  const now = validNow(dependencies.now);
  ...
  try {
    return await dependencies.database.$transaction(async (transaction) => {
      const loaded = await loadLockedMutationJob(
        transaction,
        dependencies.actor,
        envelope.data.jobId,
        now,
      ...
      if (
        !canMutateJob(dependencies.actor, loaded.assignments[0]?.role ?? null)
      )
        return failure("NOT_FOUND");
      if (loaded.status !== "DRAFT" && loaded.status !== "CHANGES_REQUESTED")
        return failure("CONFLICT");
      if (loaded.statusEvents.length > 0)
        return currentVersionSuccess(loaded, true);
      const revision = loaded.currentRevision;
      if (revision === null) return failure("CONFLICT");
      if (
        loaded.version !== envelope.data.expectedJobVersion ||
        revision.version !== envelope.data.expectedRevisionVersion
      ) {
        return failure("CONFLICT");
      }
      if (input.step === 1) {
        ...
        if (
          step.validThrough !== null &&
          !isFuturePublicationDate(step.validThrough, now)
        )
          return failure("INVALID_INPUT", ["validThrough"]);
        if (!(await validateStepOneCatalog(transaction, step)))
          return failure("INVALID_INPUT", ["catalog"]);
      }
      if (input.step === 2) {
        const step = jobWizardStepTwoSchema.parse(input.data);
        if (!(await validateSkillCatalog(transaction, step.skillIds)))
          return failure("INVALID_INPUT", ["skillIds"]);
      }

      if (revision.submittedAt !== null) {
        if (loaded.status !== "CHANGES_REQUESTED") return failure("CONFLICT");
        const cloned = await createEditableClone(
          transaction,
          loaded,
        ...
        await writeDraftUpdatedEvidence(
          transaction,
          dependencies,
          now,
          loaded,
        ...
        return success({
          ...
          jobVersion: loaded.version + 1,
          revisionVersion: 1,
        });
      }

      const nextRevision = applyStepToRevision(
        revision,
        input.step,
        dataResult.data,
      );
      const revisionUpdate = await transaction.jobRevision.updateMany({
        ...
        data: {
          ...nextRevision.data,
          contentChecksum: checksumRevision(nextRevision.checksum),
          version: { increment: 1 },
        },
      });
      if (revisionUpdate.count !== 1) return failure("CONFLICT");
      await replaceStepChildren(
        transaction,
        revision.id,
        input.step,
        dataResult.data,
...
export async function runEmployerJobReportingCheck(
  ...
  EmployerJobCommandResult<JobVersionValue & Readonly<{ checkId: string }>>
> {
  const parsed = jobCommandEnvelopeSchema
    .extend({ occupationCodeId: z.uuid() })
    .safeParse(input);
  if (!parsed.success || dependencies.jobroomProvider === undefined)
  ...
  const preflight = await dependencies.database.job.findFirst({
    ...
    select: {
      ...
      assignments: {
        where: currentAssignmentWhere(actor, now),
        take: 1,
        select: { role: true },
      },
    },
  ...
  if (
    preflight === null ||
    !canMutateJob(actor, preflight.assignments[0]?.role ?? null)
  )
    return failure("NOT_FOUND");
  if (
    preflight.statusEvents.length > 0 &&
    preflight.currentRevision?.reportingChecks[0] !== undefined
  ) {
    ...
    if (
      versioned?.currentRevision === null ||
      versioned?.currentRevision === undefined
    )
      return failure("CONFLICT");
    return success(
      {
        jobId: versioned.id,
        revisionId: versioned.currentRevision.id,
        jobVersion: versioned.version,
        revisionVersion: versioned.currentRevision.version,
        checkId: preflight.currentRevision.reportingChecks[0].id,
      },
      true,
    );
  }
  const selectedOccupation =
    await dependencies.database.occupationCode.findFirst({
      where: { id: parsed.data.occupationCodeId },
      select: { code: true },
    });
  if (selectedOccupation === null) return failure("PROVIDER_MISMATCH");
  const providerResult =
    await dependencies.jobroomProvider.checkReportingObligation({
  ...
  try {
    return await dependencies.database.$transaction(async (transaction) => {
      ...
      if (
        loaded === null ||
        !canMutateJob(actor, loaded.assignments[0]?.role ?? null)
      )
        return failure("NOT_FOUND");
      if (loaded.status !== "DRAFT" && loaded.status !== "CHANGES_REQUESTED")
        return failure("CONFLICT");
      if (loaded.statusEvents.length > 0) {
        ...
        return check === undefined
          ...
          : success(
              {
                jobId: loaded.id,
                revisionId: loaded.currentRevision!.id,
                jobVersion: loaded.version,
                revisionVersion: loaded.currentRevision!.version,
                checkId: check.id,
              },
              true,
            );
      ...
      const revision = loaded.currentRevision;
      if (revision === null || revision.submittedAt !== null)
        return failure("CONFLICT");
      if (
        loaded.version !== parsed.data.expectedJobVersion ||
      ...
      const code = await transaction.occupationCode.findFirst({
        where: {
          ...
          occupationCodeVersion: {
            version: providerResult.datasetVersion,
            datasetYear: providerResult.dataYear,
          },
        },
        select: {
          id: true,
      ...
      if (revisionUpdate.count !== 1 || jobUpdate.count !== 1)
        throw new EmployerJobCommandError("CONFLICT");
      await transaction.jobStatusEvent.create({
        data: {
          jobId: loaded.id,
          jobRevisionId: revision.id,
...
export const EMPLOYER_JOB_AI_OPERATIONS = [
  "IMPROVE",
  "INCLUSIVE",
...
export type EmployerJobAiOperation =
  (typeof EMPLOYER_JOB_AI_OPERATIONS)[number];

export async function getEmployerJobAiSuggestion(
  input: Readonly<{
    jobId: string;
    operation: EmployerJobAiOperation;
    text: string;
  }>,
  dependencies: EmployerJobCommandDependencies,
): Promise<EmployerJobCommandResult<Readonly<{ suggestion: string }>>> {
  const parsed = employerJobAiSuggestionSchema.safeParse(input);
  if (!parsed.success || dependencies.aiProvider === undefined)
    return failure("INVALID_INPUT");
  const detail = await getEmployerJobDetail(
    dependencies.actor,
    parsed.data.jobId,
    dependencies.database,
    validNow(dependencies.now),
  );
  if (
    detail === null ||
    detail.access !== "FULL" ||
    !detail.capabilities.mutateDraft ||
    detail.revision === null
  )
    return failure("NOT_FOUND");
  if (detail.status !== "DRAFT" && detail.status !== "CHANGES_REQUESTED")
    return failure("CONFLICT");
  const text = stripUnsafeHtml(parsed.data.text);
  let suggestion: string;
  if (parsed.data.operation === "IMPROVE")
    suggestion = await dependencies.aiProvider.improveJobText(text);
  else if (parsed.data.operation === "INCLUSIVE")
    suggestion = await dependencies.aiProvider.rewriteInclusive(text);
  else if (parsed.data.operation === "SHORTEN_REQUIREMENTS")
    suggestion = await dependencies.aiProvider.shortenRequirements(text);
  else {
    const revision = detail.revision;
    const suggestions =
      await dependencies.aiProvider.suggestFairScoreImprovements({
    ...
    suggestion =
      suggestions.find((item) => /Lohnspanne|Lohntransparenz/iu.test(item)) ??
      "Die aktuelle Lohnspanne ist vollständig; es wurde kein zusätzlicher Hinweis erzeugt.";
  }
  ...
  try {
    return await dependencies.database.$transaction(async (transaction) => {
      ...
      if (
        ...
        return failure("NOT_FOUND");
      if (loaded.statusEvents.length > 0 && loaded.status === "SUBMITTED")
        return currentVersionSuccess(loaded, true);
      const revision = loaded.currentRevision;
      if (revision === null || revision.submittedAt !== null)
        return failure("CONFLICT");
      if (
        ...
        return failure("CONFLICT");
      const actorCapability = jobActorCapability(
        dependencies.actor,
        loaded.assignments[0]?.role ?? null,
      );
      ...
      if (decision.type !== "OK")
        return failure(decision.type === "CONFLICT" ? "CONFLICT" : "FORBIDDEN");
      ...
      const revisionUpdate = await transaction.jobRevision.updateMany({
        ...
        data: { submittedAt: now, version: { increment: 1 } },
      });
      const jobUpdate = await transaction.job.updateMany({
        where: {
          id: loaded.id,
...
export async function pauseAndCreateEmployerJobRevision(
  ...
  if (!parsed.success) return failure("INVALID_INPUT");
  const now = validNow(dependencies.now);
  const pauseEventKey = operationKey(
    "job-pause-edit",
    parsed.data.idempotencyKey,
  );
  const draftEventKey = operationKey(
    "job-pause-edit-draft",
    parsed.data.idempotencyKey,
  );
  try {
    return await dependencies.database.$transaction(async (transaction) => {
      ...
      if (loaded === null || !canManageJobLifecycle(dependencies.actor))
        return failure("NOT_FOUND");
      if (loaded.statusEvents.length > 0 && loaded.status === "DRAFT")
        return currentVersionSuccess(loaded, true);
      const revision = loaded.currentRevision;
      if (
        revision === null ||
        loaded.currentRevisionId !== loaded.publishedRevisionId
      )
        return failure("CONFLICT");
      if (
        loaded.version !== parsed.data.expectedJobVersion ||
      ...
      const pauseDecision = decideJobTransition({
        action: "PAUSE_FOR_MATERIAL_EDIT",
        actor: actorCapability,
        currentStatus: loaded.status as JobStatus,
      });
      if (pauseDecision.type !== "OK") return failure("CONFLICT");
      const draftDecision = decideJobTransition({
        action: "CREATE_REVISION_FROM_PAUSED",
        actor: actorCapability,
        currentStatus: "PAUSED",
      });
      if (draftDecision.type !== "OK") return failure("CONFLICT");
      const cloned = await cloneRevision(
        transaction,
        loaded,
        dependencies.actor.userId,
      );
      const updated = await transaction.job.updateMany({
        ...
        data: {
          status: "DRAFT",
          currentRevisionId: cloned.id,
          version: { increment: 1 },
        },
      });
      if (updated.count !== 1) throw new EmployerJobCommandError("CONFLICT");
      await deactivatePublicationFingerprint(transaction, loaded.id, now);
      await transaction.jobStatusEvent.createMany({
        data: [
          {
            jobId: loaded.id,
            jobRevisionId: revision.id,
            ...
            idempotencyKey: pauseEventKey,
            correlationId: dependencies.correlationId,
          },
          {
            jobId: loaded.id,
            jobRevisionId: cloned.id,
            kind: "REVISION_REOPENED",
            fromStatus: "PAUSED",
            toStatus: "DRAFT",
            actorUserId: dependencies.actor.userId,
            reasonCode: "MATERIAL_EDIT",
            idempotencyKey: draftEventKey,
            correlationId: dependencies.correlationId,
          },
        ],
      });
...
async function createEmployerJobRevisionFromTerminal(
  ...
  const now = validNow(dependencies.now);
  const eventKey = operationKey(
    sourceStatus === "PAUSED" ? "job-clone-paused" : "job-clone-rejected",
    parsed.data.idempotencyKey,
  );
  try {
    return await dependencies.database.$transaction(async (transaction) => {
      ...
      if (loaded.statusEvents.length > 0 && loaded.status === "DRAFT")
        return currentVersionSuccess(loaded, true);
      const revision = loaded.currentRevision;
      if (revision === null || loaded.status !== sourceStatus)
        return failure("CONFLICT");
      if (
        loaded.version !== parsed.data.expectedJobVersion ||
        revision.version !== parsed.data.expectedRevisionVersion
        ...
        data: {
          ...
          reasonCode:
            sourceStatus === "REJECTED"
              ? "REJECTED_REVISION_CLONED"
              : "PAUSED_REVISION_CLONED",
          idempotencyKey: eventKey,
          correlationId: dependencies.correlationId,
        },
      });
...
export async function reactivateEmployerJob(
  ...
  const preflight = await dependencies.database.job.findFirst({
    where: {
      id: parsed.data.jobId,
      ...buildEmployerJobScopeWhere(dependencies.actor, now, {
        activeCompanyOnly: true,
      }),
    },
    select: {
      ...
      currentRevision: {
        select: { id: true, version: true, validThrough: true },
      },
      statusEvents: {
        where: { idempotencyKey: eventKey },
        take: 1,
  ...
  if (preflight === null || preflight.currentRevision === null)
    return failure("NOT_FOUND");
  ...
  if (
    preflight.status !== "PAUSED" ||
    preflight.currentRevisionId !== preflight.publishedRevisionId
  )
    return failure("CONFLICT");
  const port = createPrismaPublishQuotaPort(
    ...
    async (transaction) => {
      ...
      if (loaded === null || !canManageJobLifecycle(dependencies.actor))
        return failure("NOT_FOUND");
      if (loaded.statusEvents.length > 0 && loaded.status === "PUBLISHED")
        return currentVersionSuccess(loaded, true);
      const revision = loaded.currentRevision;
      if (
        revision === null ||
      ...
      if (
        ...
        !isFuturePublicationDate(revision.validThrough, now)
      )
        return failure("CONFLICT");
      ...
      if (actorCapability === null) return failure("FORBIDDEN");
      const decision = decideJobTransition({
        action: "REACTIVATE_UNCHANGED",
        actor: actorCapability,
        currentStatus: "PAUSED",
      });
      if (decision.type !== "OK") return failure("CONFLICT");
      const freshness = await establishPublicationFreshness(transaction, {
        jobId: loaded.id,
        revisionId: revision.id,
        companyId: dependencies.actor.companyId,
        sourceScope: buildJobPublicationSourceScopeV1(loaded),
        publishedAt: now,
      ...
      if (!freshness.ok) return failure("DUPLICATE");
      const updated = await transaction.job.updateMany({
        where: {
          id: loaded.id,
          status: "PAUSED",
  ...
  try {
    return await dependencies.database.$transaction(async (transaction) => {
      ...
      if (loaded === null || !canManageJobLifecycle(dependencies.actor))
        return failure("NOT_FOUND");
      if (
        loaded.statusEvents.length > 0 &&
        loaded.status === transition.toStatus
      )
        return currentVersionSuccess(loaded, true);
      const revision = loaded.currentRevision;
      if (revision === null) return failure("CONFLICT");
...
const JOB_REVISION_SELECT = {
  ...
  skills: {
    orderBy: [{ skill: { name: "asc" as const } }, { id: "asc" as const }],
    select: {
      skillId: true,
      required: true,
  ...
  benefits: {
    ...
    select: { benefitCode: true, description: true, sortOrder: true },
  },
  reportingChecks: {
...
type JobRevisionRow = Prisma.JobRevisionGetPayload<{
  select: typeof JOB_REVISION_SELECT;
}>;
...
function mutationJobSelect(
  ...
  return {
    ...
    currentRevision: { select: JOB_REVISION_SELECT },
    revisions: {
      orderBy: [{ revisionNumber: "desc" as const }, { id: "desc" as const }],
      take: 1,
...
type MutationJobRow = Prisma.JobGetPayload<{
  select: ReturnType<typeof mutationJobSelect>;
}>;
...
async function loadLockedMutationJob(
  ...
  const where = {
    ...
    ...buildEmployerJobScopeWhere(actor, now, {
      assignmentRoles:
        actor.membershipRole === "RECRUITER" ? assignmentRoles : undefined,
      activeCompanyOnly,
    }),
  };
  const first = await transaction.job.findFirst({
    where,
    select: mutationJobSelect(actor, now, eventKey),
  });
...
function initialRevisionSnapshot(
  ...
  const checksum = checksumRevision({
    revisionNumber,
    ...step,
    description: "",
    companyIntro: null,
...
function applyStepToRevision(
  ...
  let skills = revision.skills.map((row) => ({
    skillId: row.skillId,
    required: row.required,
  }));
  let benefits = revision.benefits.map(
    ({ benefitCode, description, sortOrder }) => ({
      benefitCode,
      description,
      sortOrder,
    }),
  ...
  if (stepNumber === 1) {
    ...
    benefits = step.benefits.map((benefit, sortOrder) => ({
      ...benefit,
      sortOrder,
    }));
  } else {
...
async function replaceStepChildren(
  ...
  if (stepNumber === 1) {
    const step = jobWizardStepOneSchema.parse(rawStep);
    await transaction.jobRevisionLanguage.deleteMany({
      where: { jobRevisionId: revisionId },
    });
    if (step.languages.length > 0)
      await transaction.jobRevisionLanguage.createMany({
        data: step.languages.map((language) => ({
          jobRevisionId: revisionId,
          ...language,
        })),
      });
  ...
  if (stepNumber === 2) {
    const step = jobWizardStepTwoSchema.parse(rawStep);
    await transaction.jobRevisionBenefit.deleteMany({
      where: { jobRevisionId: revisionId },
    });
    await transaction.jobRevisionSkill.deleteMany({
      where: { jobRevisionId: revisionId },
    });
    if (step.benefits.length > 0)
      await transaction.jobRevisionBenefit.createMany({
        data: step.benefits.map((benefit, sortOrder) => ({
          jobRevisionId: revisionId,
          ...benefit,
          sortOrder,
        })),
      });
    if (step.skillIds.length > 0)
      await transaction.jobRevisionSkill.createMany({
        data: step.skillIds.map((skillId) => ({
          jobRevisionId: revisionId,
          skillId,
          required: true,
        })),
...
async function createEditableClone(
  ...
  const nextRevisionNumber =
    (job.revisions[0]?.revisionNumber ?? source.revisionNumber) + 1;
...
async function copyRevisionChildren(
  ...
  const benefits =
    stepNumber === 2
      ? jobWizardStepTwoSchema
          .parse(rawStep)
          .benefits.map((benefit, sortOrder) => ({ ...benefit, sortOrder }))
      : source.benefits;
  const skills =
    stepNumber === 2
      ? jobWizardStepTwoSchema
          .parse(rawStep)
          .skillIds.map((skillId) => ({ skillId, required: true }))
      : source.skills;
  if (languages.length > 0)
    await transaction.jobRevisionLanguage.createMany({
      data: languages.map(({ code, minLevel }) => ({
        jobRevisionId: targetRevisionId,
        code,
        minLevel,
      })),
    });
  if (benefits.length > 0)
    await transaction.jobRevisionBenefit.createMany({
      data: benefits.map(({ benefitCode, description, sortOrder }) => ({
        jobRevisionId: targetRevisionId,
        benefitCode,
        description,
        sortOrder,
...
function revisionChecksumInput(revision: JobRevisionRow) {
  return {
    ...
    skills: revision.skills.map(({ skillId, required }) => ({
      skillId,
      required,
    })),
...
function checksumRevision(value: unknown) {
  return createHash("sha256")
    .update(stableSerialize({ schema: "employer-job-revision/v1", value }))
    .digest("hex");
}
...
async function validateStepOneCatalog(
  transaction: Prisma.TransactionClient,
  step: JobWizardStepOne,
) {
  const [category, canton, city] = await Promise.all([
    transaction.category.findFirst({
      where: { id: step.categoryId, isActive: true },
      select: { id: true },
    }),
    step.cantonId === null
      ? Promise.resolve(null)
      : transaction.canton.findFirst({
          where: { id: step.cantonId, isActive: true },
          select: { id: true },
        }),
    step.cityId === null
      ? Promise.resolve(null)
      : transaction.city.findFirst({
          where: {
            id: step.cityId,
            cantonId: step.cantonId ?? undefined,
            isActive: true,
  ...
  return (
    category !== null &&
    (step.cantonId === null || canton !== null) &&
    (step.cityId === null || city !== null)
  );
}

async function validateSkillCatalog(
  transaction: Prisma.TransactionClient,
  skillIds: readonly string[],
) {
  if (skillIds.length === 0) return true;
  return (
    (await transaction.skill.count({
  ...
  for (const result of [stepOne, stepTwo, stepThree]) {
    if (!result.success)
      for (const issue of result.error.issues)
        issues.add(issue.path.join(".") || "revision");
  }
  if (!isFuturePublicationDate(revision.validThrough, now))
    issues.add("validThrough");
...
function toRevisionDetail(revision: JobRevisionRow): EmployerJobRevisionDetail {
  ...
  return Object.freeze({
    ...
    skills: Object.freeze(
      revision.skills.map(({ skillId, skill, required }) => ({
        id: skillId,
        name: skill.name,
        required,
      })),
...
function currentVersionSuccess(
  job: MutationJobRow,
  replay: boolean,
): EmployerJobCommandResult<JobVersionValue> {
  const revision = job.currentRevision;
...
function canMutateJob(actor: EmployerJobActor, assignmentRole: string | null) {
  return (
    ...
    (actor.membershipRole === "RECRUITER" && assignmentRole === "EDITOR")
  );
}
...
function jobActorCapability(
  actor: EmployerJobActor,
  assignmentRole: string | null,
): JobActorCapability | null {
  if (actor.membershipRole === "OWNER") return "COMPANY_OWNER";
  if (actor.membershipRole === "ADMIN") return "COMPANY_ADMIN";
  if (actor.membershipRole === "RECRUITER" && assignmentRole === "EDITOR")
    return "RECRUITER_EDITOR";
  return null;
}
...
function isFuturePublicationDate(value: Date | null, now: Date): value is Date {
  return (
    ...
    value.getTime() <= now.getTime() + MAX_PUBLICATION_DAYS * DAY
  );
}
...
function zodIssues(result: { success: boolean; error?: z.ZodError }) {
  return result.success
    ? undefined
    : result.error?.issues.map((issue) => issue.path.join(".") || "input");
}
...
function commandFailureFromError<TValue>(
  error: unknown,
): EmployerJobCommandResult<TValue> {
  return error instanceof EmployerJobCommandError
    ? failure(error.code)
