...
export async function listAdminContent(dependencies: AdminDependencies) {
  ...
  const [pages, assessments, cantons, categories] = await Promise.all([
    dependencies.database.contentPage.findMany({
    ...
    dependencies.database.clusterLaunchAssessment.findMany({
      ...
      select: {
        ...
        canton: { select: { name: true, code: true } },
        category: { select: { name: true } },
      },
    }),
    dependencies.database.canton.findMany({
      where: { isActive: true },
      orderBy: [{ sortOrder: "asc" }, { name: "asc" }],
      select: { id: true, code: true, name: true },
    }),
    dependencies.database.category.findMany({
      where: { isActive: true },
      orderBy: [{ sortOrder: "asc" }, { name: "asc" }],
...
export async function getAdminContentPage(
  ...
  if (
    !(await requireCapability(dependencies, "ADMIN_CONTENT_MANAGE")) ||
    !z.uuid().safeParse(pageId).success
  )
    return null;
  return dependencies.database.contentPage.findUnique({
    where: { id: pageId },
    select: {
      id: true,
      slug: true,
...
export async function saveContentDraft(
  ...
  if (body.length < 20 || /<script|onerror\s*=|javascript\s*:/iu.test(body))
  ...
  if (
    parsed.data.canonicalPath !== expectedPath ||
    !parsed.data.canonicalPath.startsWith("/")
  )
    return adminFailure("INVALID_INPUT");
  ...
  try {
    return await dependencies.database.$transaction(
      async (transaction) => {
        const replay = await transaction.contentEvent.findUnique({
          where: { idempotencyKey: eventKey },
          select: {
            contentPageId: true,
            contentRevisionId: true,
            contentRevision: { select: { revisionNumber: true, status: true } },
          },
        });
        if (replay !== null)
          return adminSuccess(
            {
              pageId: replay.contentPageId,
              revisionId: replay.contentRevisionId,
              revisionNumber: replay.contentRevision.revisionNumber,
              status: replay.contentRevision.status,
            },
            true,
          );
        ...
        if (pageId === undefined) {
          ...
          if (
            ...
            page.locale !== parsed.data.locale ||
            page.type !== parsed.data.type ||
            page.canonicalPath !== parsed.data.canonicalPath
          )
        ...
        const revision = await transaction.contentRevision.create({
          data: {
            ...
            excerpt: parsed.data.excerpt,
            body,
            authoredByUserId: dependencies.actor.userId,
            contentHash,
            version: 1,
            createdAt: now,
          },
        ...
        await transaction.contentEvent.create({
          data: {
            id: randomUUID(),
            contentPageId: pageId,
        ...
        await writeAdminAudit(transaction, dependencies, now, {
          ...
          capability: "ADMIN_CONTENT_MANAGE",
...
export async function transitionContentRevision(
  ...
  const now = adminNow(dependencies.now);
  const eventKey = operationKey(
    `admin-content-${parsed.data.action.toLowerCase()}`,
    parsed.data.idempotencyKey,
  );
  const rule =
    parsed.data.action === "SUBMIT"
      ? {
          ...
          audit: "CONTENT_REVIEWED" as const,
        }
        ...
        : parsed.data.action === "REJECT"
          ...
          : parsed.data.action === "PUBLISH"
            ? {
                ...
                audit: "CONTENT_PUBLISHED" as const,
              }
            : {
                from: "PUBLISHED" as const,
                to: "UNPUBLISHED" as const,
                kind: "UNPUBLISHED" as const,
                audit: "CONTENT_UNPUBLISHED" as const,
              };
  try {
    return await dependencies.database.$transaction(
      async (transaction) => {
        ...
        const revision = await transaction.contentRevision.findUnique({
          ...
          select: {
            ...
            status: true,
            version: true,
            contentPage: { select: { currentPublishedRevisionId: true } },
            events: {
              where: { idempotencyKey: eventKey },
              take: 1,
              select: { id: true },
        ...
        if (revision.events.length > 0 && revision.status === rule.to)
          return adminSuccess(
            {
              revisionId: revision.id,
              status: rule.to,
        ...
        if (
          revision.status !== rule.from ||
          revision.version !== parsed.data.expectedVersion
        )
          return adminFailure("CONFLICT");
        if (
          parsed.data.action === "UNPUBLISH" &&
          revision.contentPage.currentPublishedRevisionId !== revision.id
        )
          return adminFailure("CONFLICT");
        const changed = await transaction.contentRevision.updateMany({
          where: {
            id: revision.id,
            status: rule.from,
            version: revision.version,
          ...
          data: {
            status: rule.to,
            version: { increment: 1 },
            ...(parsed.data.action === "APPROVE" ? { reviewedAt: now } : {}),
            ...(parsed.data.action === "PUBLISH" ? { publishedAt: now } : {}),
          },
        });
        if (changed.count !== 1) throw new AdminDomainError("CONFLICT");
        if (parsed.data.action === "PUBLISH")
          await transaction.contentPage.update({
            where: { id: revision.contentPageId },
            data: { currentPublishedRevisionId: revision.id, updatedAt: now },
          });
        if (parsed.data.action === "UNPUBLISH") {
          await transaction.contentPage.update({
            where: { id: revision.contentPageId },
            data: { currentPublishedRevisionId: null, updatedAt: now },
          });
        }
        await transaction.contentEvent.create({
          data: {
            id: randomUUID(),
            contentPageId: revision.contentPageId,
            contentRevisionId: revision.id,
            kind: rule.kind,
            actorUserId: dependencies.actor.userId,
...
export async function getClusterAssessmentDetail(
  dependencies: AdminDependencies,
  assessmentId: string,
) {
  if (
    ...
    return null;
  return dependencies.database.clusterLaunchAssessment.findUnique({
    ...
    select: {
      ...
      validUntil: true,
      status: true,
      productApprovedByUserId: true,
      productApprovedAt: true,
      opsApprovedByUserId: true,
      opsApprovedAt: true,
      activatedAt: true,
      revokedAt: true,
      canton: { select: { code: true, name: true } },
      category: { select: { name: true, slug: true } },
      searchQualityEvidence: {
        select: {
          selectionStatus: true,
          searchPolicyVersion: true,
          taxonomyVersion: true,
          rankingVersion: true,
          precisionAt10Bps: true,
          recallAt10Bps: true,
          coverageBps: true,
          relevantTopKJobs: true,
          expertReviewedAt: true,
          expertReviewerUserId: true,
        },
      },
      events: {
        orderBy: [{ createdAt: "asc" }, { id: "asc" }],
        select: { kind: true, reasonCode: true, createdAt: true },
      },
    },
  });
}

const clusterActionSchema = z.strictObject({
  assessmentId: z.uuid(),
  action: z.enum(["PRODUCT_APPROVE", "OPS_APPROVE", "ACTIVATE", "REVOKE"]),
  reasonCode: z
    .string()
    .trim()
    .regex(/^[A-Z][A-Z0-9_]{1,63}$/u),
...
export async function approveClusterForProduct(
  raw: unknown,
  dependencies: AdminDependencies,
) {
...
export async function approveClusterForOps(
  raw: unknown,
  dependencies: AdminDependencies,
) {
...
export async function revokeCluster(
  raw: unknown,
  dependencies: AdminDependencies,
) {
...
async function namedClusterTransition(
  raw: unknown,
  action: "PRODUCT_APPROVE" | "OPS_APPROVE" | "ACTIVATE" | "REVOKE",
  dependencies: AdminDependencies,
) {
...
export async function transitionClusterLaunch(
  raw: unknown,
  dependencies: AdminDependencies,
) {
  ...
  const capability: AdminCapability =
    parsed.data.action === "PRODUCT_APPROVE"
      ? "ADMIN_CLUSTER_PRODUCT_APPROVE"
      : parsed.data.action === "OPS_APPROVE"
  ...
  const now = adminNow(dependencies.now);
  const eventKey = operationKey(
    `admin-cluster-${parsed.data.action.toLowerCase()}`,
    parsed.data.idempotencyKey,
  );
  try {
    return await dependencies.database.$transaction(
      async (transaction) => {
        ...
        if (assessment.events.length > 0)
          return adminSuccess(
            { assessmentId: assessment.id, status: assessment.status },
            true,
          );
        if (
          parsed.data.action === "ACTIVATE" &&
          assessment.status === "ACTIVATED"
        )
          return adminSuccess(
            { assessmentId: assessment.id, status: assessment.status },
            true,
          );
        if (parsed.data.action === "REVOKE" && assessment.status === "REVOKED")
          return adminSuccess(
            { assessmentId: assessment.id, status: assessment.status },
            true,
          );
        ...
        if (parsed.data.action === "PRODUCT_APPROVE") {
          if (status !== "READY" || assessment.productApprovedAt !== null)
            return adminFailure("CONFLICT");
          kind = "PRODUCT_APPROVED";
          data = {
            productApprovedByUserId: dependencies.actor.userId,
            productApprovedAt: now,
          };
        } else if (parsed.data.action === "OPS_APPROVE") {
          if (status !== "READY" || assessment.opsApprovedAt !== null)
            return adminFailure("CONFLICT");
          kind = "OPS_APPROVED";
          data = {
            opsApprovedByUserId: dependencies.actor.userId,
            opsApprovedAt: now,
          };
        } else if (parsed.data.action === "ACTIVATE") {
          const knownPolicy =
            assessment.policyVersion === CLUSTER_LAUNCH_POLICY_V1.version ||
            assessment.policyVersion === CLUSTER_LAUNCH_POLICY_V2.version;
          const v2Allowed =
            assessment.policyVersion !== CLUSTER_LAUNCH_POLICY_V2.version ||
            decideClusterV2Activation(assessment.searchQualityEvidence).allowed;
          if (
            status !== "READY" ||
            !knownPolicy ||
            !v2Allowed ||
            assessment.dataProvenance !== "LIVE" ||
            assessment.evaluatedAt > now ||
            assessment.validUntil <= now ||
            assessment.productApprovedAt === null ||
            assessment.opsApprovedAt === null ||
            !clusterMetricsPass(assessment)
          )
            return adminFailure("CONFLICT");
          const superseded =
            await transaction.clusterLaunchAssessment.findFirst({
              where: {
                id: { not: assessment.id },
                cantonId: assessment.cantonId,
          ...
          if (superseded !== null) {
            await transaction.clusterLaunchAssessment.update({
              where: { id: superseded.id },
              data: {
                status: "REVOKED",
                revokedAt: now,
                revokeReason: "SUPERSEDED_BY_NEW_ASSESSMENT",
              },
            });
            await transaction.clusterLaunchEvent.create({
              data: {
                id: randomUUID(),
                clusterLaunchAssessmentId: superseded.id,
                kind: "REVOKED",
                actorUserId: dependencies.actor.userId,
                reasonCode: "SUPERSEDED_BY_NEW_ASSESSMENT",
                correlationId: `${eventKey}:supersede`.slice(0, 128),
                createdAt: now,
              },
            });
          }
          ...
          data = {
            status,
            activatedAt: now,
            activationReason: parsed.data.reasonCode,
          };
        } else {
          ...
          data = {
            status,
            revokedAt: now,
            revokeReason: parsed.data.reasonCode,
          };
        }
        await transaction.clusterLaunchAssessment.update({
        ...
        const auditAction =
          parsed.data.action === "ACTIVATE"
            ? ("CLUSTER_ACTIVATED" as const)
            : parsed.data.action === "REVOKE"
              ? ("CLUSTER_REVOKED" as const)
              : ("CLUSTER_ASSESSMENT_APPROVED" as const);
        await writeAdminAudit(transaction, dependencies, now, {
          action: auditAction,
          capability,
          targetType: "CLUSTER_LAUNCH_ASSESSMENT",
...
export async function projectExpiredClusterLaunches(
  raw: unknown,
  dependencies: AdminDependencies,
) {
  const parsed = clusterExpirySchema.safeParse(raw);
  if (!parsed.success) return adminFailure("INVALID_INPUT");
  if (!(await requireCapability(dependencies, "ADMIN_CLUSTER_ACTIVATE")))
    return adminFailure("FORBIDDEN");
  const now = adminNow(dependencies.now);
  const prefix = operationKey(
    "admin-cluster-expire",
    parsed.data.idempotencyKey,
  );
  try {
    return await dependencies.database.$transaction(
      async (transaction) => {
        const due = await transaction.clusterLaunchAssessment.findMany({
          where: { status: "ACTIVATED", validUntil: { lte: now } },
        ...
        for (const assessment of due) {
          ...
          const changed = await transaction.clusterLaunchAssessment.updateMany({
            where: {
              id: assessment.id,
              status: "ACTIVATED",
              validUntil: { lte: now },
          ...
          await transaction.clusterLaunchEvent.create({
            data: {
              ...
              kind: "EXPIRED",
              actorUserId: dependencies.actor.userId,
              reasonCode: "ASSESSMENT_VALIDITY_ELAPSED",
              correlationId: `${prefix}:${assessment.id}`.slice(0, 128),
              createdAt: now,
            },
          });
          projectedCount += 1;
...
function hashContent(
  value: Readonly<{ title: string; excerpt: string; body: string }>,
) {
