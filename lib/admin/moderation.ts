...
import { closeCandidateFreshnessReport } from "@/lib/jobs/freshness";
...
export async function listAdminReports(dependencies: AdminDependencies) {
  if (!(await requireCapability(dependencies, "ADMIN_REPORT_REVIEW")))
    return null;
  const now = adminNow(dependencies.now);
  return dependencies.database.abuseReport
    .findMany({
    ...
    .then((rows) =>
      rows.sort(
        (a, b) =>
          Number(a.dueAt > now) - Number(b.dueAt > now) ||
...
export async function getAdminReportDetail(
  ...
  if (
    !(await requireCapability(dependencies, "ADMIN_REPORT_REVIEW")) ||
    !z.uuid().safeParse(reportId).success
  )
    return null;
  const [report, assignableAdmins] = await Promise.all([
    dependencies.database.abuseReport.findUnique({
      where: { id: reportId },
      select: {
        id: true,
        targetType: true,
  ...
  const preview = await getLeastPrivilegeTargetPreview(
...
export async function triageAbuseReport(
  raw: unknown,
  dependencies: AdminDependencies,
) {
  ...
  try {
    return await dependencies.database.$transaction(
      async (transaction) => {
        ...
        const report = await transaction.abuseReport.findUnique({
        ...
        if (
          report.version !== parsed.data.expectedVersion ||
        ...
        const dueAt = tightenSlaDueAt(
          report.dueAt,
          report.createdAt,
          abuseSlaKey(parsed.data.severity),
        );
        const changed = await transaction.abuseReport.updateMany({
          where: {
            id: report.id,
        ...
        await transaction.abuseReportEvent.create({
          data: {
            id: randomUUID(),
            ...
            safeNote: parsed.data.safeNote ?? null,
        ...
        if (
          ...
          await transaction.abuseReportEvent.create({
            data: {
              ...
              safeNote: null,
              correlationId: dependencies.correlationId,
              idempotencyKey: `${eventKey}:assignment`,
              createdAt: now,
            },
          });
        }
...
export async function applyModerationRestriction(
  raw: unknown,
  dependencies: AdminDependencies,
) {
  const parsed = applyRestrictionSchema.safeParse(raw);
  if (!parsed.success) return adminFailure("INVALID_INPUT");
  if (!(await requireCapability(dependencies, "ADMIN_RESTRICTION_MANAGE")))
    return adminFailure("FORBIDDEN");
  const now = adminNow(dependencies.now);
  if (
    parsed.data.endsAt !== null &&
    parsed.data.endsAt !== undefined &&
    parsed.data.endsAt <= now
  )
    return adminFailure("INVALID_INPUT");
  const restrictionKey = operationKey(
    "admin-restriction-apply",
    parsed.data.idempotencyKey,
  );
  ...
  try {
    return await dependencies.database.$transaction(
      async (transaction) => {
        await lockReport(transaction, parsed.data.reportId);
        const replay = await transaction.moderationRestriction.findUnique({
          where: { idempotencyKey: restrictionKey },
        ...
        if (replay !== null)
          return replay.status === "ACTIVE"
            ? adminSuccess(
                {
                  restrictionId: replay.id,
                  type: replay.targetType,
                  affectedResourceId: replay.targetId,
                },
                true,
              )
        ...
        if (report === null) return adminFailure("NOT_FOUND");
        if (
          report.version !== parsed.data.expectedReportVersion ||
          ["RESOLVED", "DISMISSED"].includes(report.status)
        )
          return adminFailure("CONFLICT");
        const mapped = await resolveRestrictionTarget(
          transaction,
          report,
          parsed.data.restrictionType,
          parsed.data.affectedResourceId,
        );
        if (mapped === null) return adminFailure("CONFLICT");
        const active = await transaction.moderationRestriction.findFirst({
          where: {
            targetType: parsed.data.restrictionType,
            targetId: mapped.affectedResourceId,
            status: "ACTIVE",
          },
          select: { id: true },
        });
        if (active !== null) return adminFailure("CONFLICT");
        const restriction = await transaction.moderationRestriction.create({
          data: {
            id: randomUUID(),
            abuseReportId: report.id,
            targetType: parsed.data.restrictionType,
            ...
            appliedByUserId: dependencies.actor.userId,
            startsAt: now,
        ...
        await transaction.abuseReportEvent.create({
          data: {
            ...
            actorUserId: dependencies.actor.userId,
            reasonCode: parsed.data.restrictionType,
            safeNote: null,
            correlationId: dependencies.correlationId,
            idempotencyKey: eventKey,
            createdAt: now,
        ...
        await writeAdminAudit(transaction, dependencies, now, {
          action: "MODERATION_RESTRICTION_APPLIED",
          capability: "ADMIN_RESTRICTION_MANAGE",
          targetType: "MODERATION_RESTRICTION",
        ...
        return adminSuccess({
          ...
          type: parsed.data.restrictionType,
          affectedResourceId: mapped.affectedResourceId,
        });
      },
      { isolationLevel: "Serializable" },
    );
...
export async function liftModerationRestriction(
  raw: unknown,
  dependencies: AdminDependencies,
) {
...
export async function expireModerationRestriction(
  raw: unknown,
  dependencies: AdminDependencies,
) {
...
async function endRestriction(
  raw: unknown,
  dependencies: AdminDependencies,
  toStatus: "LIFTED" | "EXPIRED",
) {
  ...
  const now = adminNow(dependencies.now);
  const eventKey = operationKey(
    `admin-restriction-${toStatus.toLowerCase()}`,
    parsed.data.idempotencyKey,
  );
  try {
    return await dependencies.database.$transaction(
      async (transaction) => {
        ...
        const restriction = await transaction.moderationRestriction.findUnique({
          where: { id: parsed.data.restrictionId },
          select: {
            id: true,
            abuseReportId: true,
            targetType: true,
        ...
        const replay = await transaction.abuseReportEvent.findUnique({
          where: { idempotencyKey: eventKey },
          select: { id: true },
        });
        if (replay !== null && restriction.status === toStatus)
          return adminSuccess(
            { restrictionId: restriction.id, status: toStatus },
            true,
          );
        if (restriction.status !== "ACTIVE") return adminFailure("CONFLICT");
        if (
          toStatus === "EXPIRED" &&
          (restriction.endsAt === null || restriction.endsAt > now)
        )
          return adminFailure("CONFLICT");
        const mapped = await resolveStoredRestrictionTarget(
          transaction,
          restriction.targetType,
          restriction.targetId,
        );
        if (mapped === null) return adminFailure("CONFLICT");
        const changed = await transaction.moderationRestriction.updateMany({
          ...
          data:
            toStatus === "LIFTED"
              ? {
                  status: "LIFTED",
                  liftedAt: now,
                  liftedByUserId: dependencies.actor.userId,
                  liftReason: parsed.data.reasonCode,
                }
        ...
        await transaction.abuseReportEvent.create({
          data: {
            id: randomUUID(),
            abuseReportId: restriction.abuseReportId,
            kind:
              toStatus === "LIFTED"
                ? "RESTRICTION_LIFTED"
                : "RESTRICTION_EXPIRED",
        ...
        await notifyModerationChange(
          transaction,
          {
            id: restriction.abuseReportId,
            reporterUserId: restriction.abuseReport.reporterUserId,
          },
          restriction.id,
          toStatus,
          mapped,
...
async function closeReport(
  ...
  const now = adminNow(dependencies.now);
  const eventKey = operationKey(
    `admin-report-${status.toLowerCase()}`,
    parsed.data.idempotencyKey,
  );
  try {
    return await dependencies.database.$transaction(
      async (transaction) => {
        ...
        if (report === null) return adminFailure("NOT_FOUND");
        if (report.events.length > 0 && report.status === status)
          return adminSuccess({ reportId: report.id, status }, true);
        if (
          report.version !== parsed.data.expectedVersion ||
          ["RESOLVED", "DISMISSED"].includes(report.status)
        )
        ...
        await transaction.abuseReportEvent.create({
          data: {
            ...
            kind: status,
            actorUserId: dependencies.actor.userId,
            reasonCode: parsed.data.resolutionCode,
            correlationId: dependencies.correlationId,
            idempotencyKey: eventKey,
            createdAt: now,
          },
        });
        await closeCandidateFreshnessReport(transaction, {
          abuseReportId: report.id,
          status,
          actorUserId: dependencies.actor.userId,
          correlationId: dependencies.correlationId,
...
type ResolvedRestrictionTarget = Readonly<{
...
async function resolveRestrictionTarget(
  transaction: Prisma.TransactionClient,
  report: ReportTarget,
  restrictionType: (typeof restrictionTypes)[number],
  affectedResourceId: string,
): Promise<ResolvedRestrictionTarget | null> {
  if (restrictionType === "HIDE_JOB") {
    if (report.targetType !== "JOB" || report.targetId !== affectedResourceId)
      return null;
    const job = await transaction.job.findUnique({
      where: { id: affectedResourceId },
  ...
  if (restrictionType === "PAUSE_COMPANY") {
    if (
      ...
      report.targetId === affectedResourceId
    ) {
    ...
    if (report.targetType === "JOB") {
      ...
      return job?.companyId === affectedResourceId
        ? {
            affectedResourceId,
            companyId: affectedResourceId,
            userId: null,
  ...
  if (restrictionType === "SUSPEND_USER") {
    ...
    if (report.targetType === "MESSAGE") {
      const message = await transaction.message.findUnique({
        ...
        select: {
          senderUserId: true,
          conversation: { select: { companyId: true, id: true } },
        },
      });
      return message?.senderUserId === affectedResourceId
        ? {
            affectedResourceId,
  ...
  return message?.conversationId === affectedResourceId
    ? {
        affectedResourceId,
        companyId: message.conversation.companyId,
        userId: null,
        conversationId: affectedResourceId,
      }
    : null;
}

async function resolveStoredRestrictionTarget(
  transaction: Prisma.TransactionClient,
  type: (typeof restrictionTypes)[number],
  targetId: string,
): Promise<ResolvedRestrictionTarget | null> {
  if (type === "HIDE_JOB") {
    const job = await transaction.job.findUnique({
...
async function applyRestrictionEffect(
  transaction: Prisma.TransactionClient,
  type: (typeof restrictionTypes)[number],
  target: ResolvedRestrictionTarget,
  now: Date,
  dependencies: AdminDependencies,
) {
  if (type === "PAUSE_COMPANY" && target.companyId !== null) {
    ...
    if (company?.status === "ACTIVE") {
      ...
      await transaction.companyStatusEvent.create({
        data: {
          ...
          toStatus: "SUSPENDED",
          actorUserId: dependencies.actor.userId,
          reasonCode: "MODERATION_RESTRICTION",
          correlationId: dependencies.correlationId,
          createdAt: now,
        },
      });
    ...
    const jobs = await transaction.job.findMany({
      where: { companyId: target.companyId, status: "PUBLISHED" },
      select: { id: true, currentRevisionId: true, version: true },
    });
    for (const job of jobs) {
      ...
      await transaction.jobStatusEvent.create({
        data: {
          ...
          actorUserId: dependencies.actor.userId,
          reasonCode: "MODERATION_RESTRICTION",
          idempotencyKey: `moderation:${dependencies.correlationId}:${job.id}`,
          correlationId: dependencies.correlationId,
          createdAt: now,
        },
      });
  ...
  if (type === "SUSPEND_USER" && target.userId !== null) {
    ...
    const user = await transaction.user.findUnique({
      ...
      select: { status: true, candidateProfile: { select: { id: true } } },
    });
...
async function notifyModerationChange(
  transaction: Prisma.TransactionClient,
  report: Readonly<{ id: string; reporterUserId: string | null }>,
  restrictionId: string,
  status: "APPLIED" | "LIFTED" | "EXPIRED",
  target: ResolvedRestrictionTarget,
) {
  ...
  if (target.conversationId !== null) {
    ...
    for (const participant of participants)
      if (participant.userId !== null) recipients.add(participant.userId);
  }
  for (const recipientUserId of recipients) {
    await writeNotificationExactlyOnce(
      ...
      {
        recipientUserId,
        kind: "MODERATION_CHANGED",
        dedupeKey: `moderation:${restrictionId}:${status}`,
        payload: { reportId: report.id, restrictionId, status },
      },
    );
  }
}

async function getLeastPrivilegeTargetPreview(
...
export async function isConversationMessageBlocked(
  database: AdminDependencies["database"] | Prisma.TransactionClient,
  ...
  return (
    (await database.moderationRestriction.count({
      where: {
        targetType: "BLOCK_MESSAGE_THREAD",
