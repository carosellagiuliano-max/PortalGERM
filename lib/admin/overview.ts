...
export async function getAdminOverview(dependencies: AdminDependencies) {
  if (!(await requireCapability(dependencies, "ADMIN_OVERVIEW_READ")))
    return null;
  ...
  const [
    ...
    dependencies.database.job.count({
      where: {
        ...
        updatedAt: { lte: ageingJobBoundary },
      },
    }),
    dependencies.database.companyVerificationRequest.count({
      where: {
        status: { in: ["PENDING", "CHANGES_REQUESTED"] },
        supersededBy: null,
      },
    }),
    dependencies.database.job.findMany({
      where: {
        status: "PUBLISHED",
        publishedAt: { lte: now },
        expiresAt: { gt: now },
    ...
    dependencies.database.supportCase.count({
      where: { status: { notIn: ["RESOLVED", "CLOSED"] }, dueAt: { lte: now } },
    }),
    dependencies.database.salesLead.count({
      where: {
        status: "NEW",
        OR: [
          { dueAt: { lte: now } },
          { dueAt: null, createdAt: { lte: leadBoundary } },
        ],
      },
    }),
    dependencies.database.auditLog.findMany({
  ...
  const activeSupply = (
    await filterPubliclyEligibleJobs(
      activeSupplyCandidates.map(({ id }) => id),
      now,
...
export async function searchAdmin(
  ...
  if (!(await requireCapability(dependencies, "ADMIN_GLOBAL_SEARCH")))
    return null;
  ...
  const jobFilters = [
    ...
    {
      currentRevision: {
        is: { title: { contains: query, mode: "insensitive" as const } },
      },
    },
    ...(isUuid ? [{ id: query }] : []),
  ];
...
export async function getOperationalQueueCounts(
  database: DatabaseClient,
  now: Date,
) {
  const [jobs, verification, reports, imports, support, leads] =
    await Promise.all([
      database.job.count({
        where: { status: { in: ["SUBMITTED", "IN_REVIEW"] } },
      }),
