...
export async function manageSalesLead(raw: unknown, dependencies: AdminDependencies) {
  ...
  try {
    return await dependencies.database.$transaction(async (transaction) => {
      ...
      const lead = await transaction.salesLead.findUnique({ where: { id: parsed.data.leadId }, select: { id: true, companyId: true, status: true, ownerUserId: true, nextAt: true } });
      ...
      const nextAt = parsed.data.action === "SET_NEXT" ? parsed.data.nextAt! : lead.nextAt;
