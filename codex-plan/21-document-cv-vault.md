...
> **Aktivierung:** SANDBOX / TEST-COHORT; LIVE UND PRODUCTION DISABLED
>
> Candidate `ca36bff59e0d759cc5243da346c6e717c650e35e` besteht den
> Quarantäne-, Scan-, Versions-, Read-Grant-, Reconciliation-, Last- und
> vollständigen G3-Vertrag. Der verschlüsselte Filesystem-Object-Store und
> Scanner sind ausschließlich Local-/CI-Sandboxadapter. Externe Provider,
> Production-Retention/Legal-Hold, autonome Worker und Bulkzugriff bleiben
> fail-closed; siehe [Phase-21-Evidence](./evidence/2026-07-26-phase-21.md).
...
- Additive Prisma-Modelle bilden `Document`, immutable
  `DocumentVersion`, Upload-Intents, append-only Scan-/Access-Evidence,
  single-use Read-Grants und Lifecycle-Outcomes ab. Legacy-Metadaten bleiben
  ausdrücklich `METADATA_ONLY_LEGACY` und werden nicht als Bytes ausgegeben.
- `LocalEncryptedDocumentObjectStore` streamt AES-256-GCM-verschlüsselt in
  ein repository-externes Sandbox-Root, veröffentlicht atomar und prüft
  Größe/SHA-256 bei Write und Read. Ungültige Konfiguration bootet
  fail-closed; es gibt keinen Real→Mock-Fallback.
- Eine geschlossene PDF-/PNG-Magic-/MIME-/Extension-Policy und der
  Sandbox-Scanner halten Spoof, Polyglot, Malware, malformed und Timeout
  nicht downloadbar. Die sieben synthetischen Fixtures sind hash-fixiert.
- JobPass bietet Upload, Fortschritt, Scan, Retry, Replace, Delete und
  verständliche Fehlerzustände. Eine Bewerbung referenziert immutable genau
  die eingereichte saubere Version; ein späterer Profil-CV-Ersatz mutiert sie
  nicht.
- Employer/Recruiter erhalten nur bei aktueller Company-, Membership-,
  Assignment-, Application- und Statusberechtigung ein maximal 60 Sekunden
  gültiges actor-gebundenes Single-use-Grant. Der Object Key ist nie
  Authorization; Admin sieht nur redigierte Zustände.
- Neun dokumentierte Handler, bounded Scan-/Reconcile-Commands, zwei
  Rate-Limit-Presets, acht redigierte Audit-Actions sowie Unit-, Provider-,
  PostgreSQL-, Production-Browser-, Mobile-/A11y- und Lasttests sind
  vorhanden. Der frühere Metadata-only-Adapter bleibt ehrlich verfügbar,
  solange der Vault deaktiviert ist.
...
Privacy-Routen. Das in
[`route-role-matrix.md`](./route-role-matrix.md) und
[`route-inventory.json`](./route-inventory.json) aufgenommene Delta umfasst:

- Candidate Status-, Upload-Intent-, Stream-, Finalize-, Scan- und
  Delete-Request-Handler aus JobPass;
- actor-gebundene single-object Grant-/Read-/Revoke-Handler aus Candidate-
  und Employer-Application;
- Lifecycle-/Hold-/Delete-/Reconciliation-Adapter ohne konkurrierende
  Phase-22-Privacy-Case-Policy;
...
ADR-032 ist für den Sandbox-Vertrag angenommen. Implementiert sind:
...
Alle Testdateien wurden angelegt und auf Candidate `ca36bff` mit Exit Code
`0` ausgeführt. Detailwerte und offen gebliebene Aktivierungsgates stehen in
der [Phase-21-Evidence](./evidence/2026-07-26-phase-21.md).
