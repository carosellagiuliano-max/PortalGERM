...
- [ ] **External-apply jobs:** if the current approved/published `JobRevision.applicationUrl` is set and URL-policy validated, CTA links out and no internal `Application` is created; record only an allowlisted external-apply click event when approved
