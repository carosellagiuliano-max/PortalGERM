...
> **Planstatus: PARTIELL / OFFEN. Technikstatus: 30A-TECHNIKBASIS UND 30D
> IMPLEMENTIERT; 30B-RECOMMENDATION-MITIGATION IMPLEMENTIERT, ADMIN-SCALE
> UNTER TRIGGER DEFERRED; 30C-MONITOR IMPLEMENTIERT, SHARDS UNTER TRIGGER
> DEFERRED. Quality-Gate: LOCAL/CI-AUTOMATION BESTANDEN; EXTERNE FACH-,
> ZIELUMGEBUNGS- UND OPERATIONS-EVIDENCE OFFEN. Aktivierung: DISABLED.**
> Vier getrennt evidenzierte Tracks verhindern falsche Prioritäten. Ein
> Trackabschluss hakt die Gesamtphase nicht pauschal ab.
...
| Track              | Plan/Technik/Quality                                                                                                                            | Aktivierungspriorität                                      |
| ------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------- |
| 30A Search Quality | Concept-/Release-/Learning-/Cluster-V2-Technik und Automation `PASS`; Pflege-Fachreview und 50k-Zielharness offen, Engineering `DISCOVERY_ONLY` | P0 für jeden tatsächlich aktivierten Cluster in LC3–LC6    |
| 30B Scale          | Recommendation-Trigger ausgelöst und Mitigation `PASS`; Adminqueues unter lokaler Baseline, Pagination/Bulk `DEFERRED / MONITORED`              | P3; P1 ab 70-%-/Budgetwarnung, P0 vor Unerreichbarkeit/LC6 |
| 30C Sitemap        | Capacity-Monitor/Worker/Thresholdtests `PASS`; Zielalertreceipt offen, Shards unter Trigger `DEFERRED / MONITORED`                              | P3 unter Trigger; Eskalation gemäss Capacity-Matrix        |
| 30D Freshness      | Projektion, Reconfirm/Filled, Report/SLA, Duplicate, Worker und Consumerparität technisch `PASS`; reale Moderationskapazität offen              | P1 bei LC2 mit realen Jobs; P0 für LC3–LC6                 |

Die technische Local-/CI-Evidence steht in
[`evidence/2026-07-29-phase-30.md`](./evidence/2026-07-29-phase-30.md).
Fachreview, Zielumgebungs-Shadow/Alerts, vierstündige Operationskapazität,
Staging und jede Cluster-/LIVE-Aktivierung bleiben `OPEN / DISABLED`.
...
- `STH-019`: ein gemeinsamer de-CH-Concept-Vertrag mit sechs Dimensionen,
  festen Releaseversionen, signiertem Cursor und Search-/Alert-/
  Recommendation-/Matching-Parität ist technisch vorhanden. Pflege ist
  `SELECTED`, aber `NOT_REVIEWED`; Engineering bleibt `DISCOVERY_ONLY`.
- `STH-020`: Adminreads besitzen weiterhin Caps 100–250. Die aktuelle
  lokale Baseline liegt unter dem 70-%-Trigger; >250-Pagination/Bulk bleibt
  deshalb ehrlich deferred und würde bei Trigger zur P1/P0-Migration.
- `STH-021`: die instrumentierte 48-Job-Empfehlungsbaseline löste den
  > 20-Query-Trigger aus. Die neue bounded Projektion benötigt zwei
  > fachliche SQL-Queries, bleibt canonical-eligibility-gebunden und liegt
  > unter dem p95-Budget.
- `STH-027`: die Single-Sitemap bleibt no-truncation/fail-closed. Ein
  registrierter Tagesmonitor persistiert Count, Bytes, Laufzeit, Wachstum,
  Forecast, Erfolg, Schwellenzustand und Owner; Zielumgebungs-Alertreceipt
  und bei Trigger Index/Shards bleiben offen.
- `STH-032`: Reconfirmation, „besetzt“, Candidate-Report, vierstündiger
  fail-closed Hold, Exact-/Near-Duplicate, Reminder/Due-Worker und ein
  gemeinsamer Eligibility-Consumervertrag sind technisch implementiert.
- `STH-036`: Search Learning persistiert keine Rohquery, verwirft PII/
  Secrets, verwendet getrennte HMAC-Buckets, `k=10`, 30-Tage-Working-State,
  einen erst am Schwellenübergang gespeicherten redigierten Reviewbegriff,
  capability-gebundenes Review/Promote/Reject, Audit und vollständige Expiry.
  Collection bleibt standardmässig deaktiviert.
