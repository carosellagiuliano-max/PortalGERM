...
> **Planungsstand:** 25. Juli 2026. Dieses Dokument plant die
...
`STH-001` bis `STH-028` nicht pauschal als Defekte behandelt, sondern nach
...
| Branch bei Analysebeginn | `codex/phase-18-release-audit` |
| HEAD bei Analysebeginn | `eb9b45ae5caca638b558f6a98e406af9ee8be0fc` (`eb9b45a`) |
| Remote-Stand bei Analysebeginn | `origin/main` und `origin/codex/phase-18-release-audit` zeigten auf `eb9b45a` |
| Lokaler Fremdumfang | die bereits vorhandene Änderung an `components/layout/brand-link.tsx` und das ungetrackte `.claude/` gehören dem Nutzer und sind kein Remediation-Befund |
| Heutige Aktivität | Repository-, Plan-, Schema-, Code-, Test- und Evidence-Analyse; keine Produktimplementierung und keine erneute vollständige Testausführung |

Die Baseline ist eine Prüfidentität, kein neuer Releasekandidat. Insbesondere
darf die historische grüne Evidence nicht so dargestellt werden, als wäre
sie am 25. Juli 2026 nochmals auf dem aktuellen, lokal nicht sauberen
Arbeitsbaum gelaufen.
...
Die Remediation trennt vier Betriebsstufen. Eine höhere Stufe erbt die
...
| --- | --- | --- | --- |
| **Lokale Demo** | Entwicklung, automatisierte Tests, Stakeholder-Demo mit klar markierten Fixtures | Mock-Provider, Production-Demo-Guard, keine echten Personen- oder Zahlungsdaten | WTP, reale Delivery, Pilot- oder Produktionsreife |
| **Beaufsichtigter Design-Partner-Test** | kleiner, invite-only Test mit benanntem Operator und manuell kontrollierten Abläufen | flowspezifische AVG-/Vertrags-/Tax-/Privacy-Freigabe, reale Einwilligung, Incident Owner, kein unbeaufsichtigter Pfad | öffentlicher Self-Service oder skalierter Betrieb |
| **Öffentlicher Produktpilot** | begrenzte echte Nutzer, produktive Provider und überwachte Kernprozesse | alle P0/P1-Findings und -Tracks des konkret freigegebenen Scopes sowie Phase 32 geschlossen; nicht benötigte P2-/Enterprise-Pfade fail-closed; nicht ausgelöste P3-Kapazitätsarbeit nur mit Headroom/Forecast/Alert/Owner deferred; autonome Worker/Outbox, Staging-/Recovery-Evidence und Provider-/Legal-Gates; bei bezahltem Scope echter Geldfluss/WTP, bei kostenlosem Scope alle Kauf-CTAs geschlossen | allgemeine Produktions- oder Skalierungsreife |
| **Produktion/Skalierung** | dauerhaft betreibbares Angebot innerhalb freigegebener Regionen und Produkte | bestätigte SLO/RPO/RTO, On-call, Datenschutz-/Retention-Lifecycle, Kapazitätsnachweis, Security-/Accessibility-/Browser-Abnahme, laufende Compliance | unbegrenzte Reichweite, nicht freigegebene Produkte oder ungeprüfte Länder-/Datenflüsse |
...
| [ ] 20 | [Identity, E-Mail und Notifications](./20-identity-email-notifications.md) | `STH-001`, `STH-002`, `STH-013`, `STH-026`; E-Mail-Anteil `STH-004` | verifizierte E-Mail-Identität, erreichbare Step-up-Challenge, idempotente Zustellung und zentrale Präferenzen |
...
| [ ] 22 | [Privacy, Legal und Analytics](./22-privacy-legal-analytics.md) | `STH-006`, `STH-007`, `STH-017` | reale Export-/Korrektur-/Löschprozesse, versionierte Rechtstexte und consent-bewusste LIVE-Analytics |
| [ ] 23 | [Production Operations und Worker](./23-production-operations-workers.md) | verbleibende Provideranteile `STH-004`, `STH-008`, `STH-009` | explizit freigegebene reale Adapter, autonome Ausführung und belastbare Staging-/Recovery-Grenzen |
| [ ] 24 | [Reales Billing und Finance](./24-real-billing-finance.md) | `STH-005`; Payment-Anteil `STH-004` | echter, webhookbasierter Geldfluss, Reconciliation, Refund/Dunning und korrekte Rechnung |
| [ ] 25 | [Admin-Security](./25-admin-security.md) | `STH-010`, `STH-011` | Least Privilege, Separation of Duties sowie Step-up/MFA |
| [ ] 26 | [Company Trust und Verifikation](./26-company-trust-verification.md) | `STH-014` | beweisgestützter, vier-Augen-fähiger Trust-Lifecycle |
...
| [ ] 28 | [Recruiting-Workflows](./28-recruiting-workflows.md) | `STH-015`, `STH-016` | ehrlicher externer Bewerbungsstatus und persistente Interviewplanung |
| [ ] 29 | [UX, Mobile und Accessibility](./29-ux-mobile-accessibility.md) | `STH-023`, `STH-025`; UX-Regression `STH-026` | bedienbare Cross-Browser-/Mobile-UX und vollständige Preference-Center-Oberflächen |
| [ ] 30 | [Startcluster-Suche und Scale Operations](./30-search-scale-operations.md) | 30A: `STH-019` P1; 30B: `STH-020/021`; 30C: `STH-027` P3/triggerbasiert | früher Startcluster-Suchvertrag, spätere skalierbare Reads/Queues/Empfehlungen sowie Capacity-Monitoring und nur bei Trigger Sitemap-Shards |
| [ ] 31 | [Monetarisierung und Marktvalidierung](./31-monetization-market-validation.md) | `STH-018`, `STH-022`, `STH-028` | früher ICP-/WTP-/Cashflow-Discovery-Track und spätere Freigabe nur tatsächlich lieferbarer Pakete |
| [ ] 32 | [Finaler Production-Release-Audit](./32-production-release-audit.md) | `STH-024` und Abschluss aller übrigen Befunde | ein exakter Commit/Artefakt mit vollständiger automatischer und manueller Evidence |
...
Incident Owner und Kill Switch.
...
**Befunde:** `STH-005` sowie der Payment-Provideranteil von `STH-004`.

**Abhängigkeiten:** 23 reale Payment-Adapter-Governance und Worker;
22 Legal/Privacy/Retention; externe Tax-/Vertrags-/AVG-Gates sowie ein
vorregistrierter Commercial-Go/No-go-Entscheid. Der frühe WTP-Track aus Phase
31 darf einen rechtlich freigegebenen manuellen Rechnungs-/Design-Partner-Test
nutzen; die belastbare Paid-WTP-Auswertung und öffentliche Angebotsfreigabe
folgen nach Phase 24 und erzeugen deshalb keinen Rückwärtsblocker.
...
Refund-/Dispute-Auswirkung auf Grants ohne Ledgerkorruption; PDF/
...
**Zweck:** Die bestehende Capability-Abstraktion wird zu echten
Least-Privilege-Adminrollen. Sensitive Aktionen verlangen Step-up/MFA;
Separation of Duties und break-glass werden explizit und beobachtbar.

**Befunde:** `STH-010`, `STH-011`.
...
MFA-Recovery und Step-up-Alter; Sessionversion bei Rollenänderung;
break-glass mit Zeitlimit, Alarm und Audit; kein globaler `ADMIN`-Fallback
bei unbekannter Capability.
...
**Zweck:** Personen können additive Plattform- und Company-Personas
...
**Zweck:** Externe Bewerbungen werden nicht mehr nur als Klick interpretiert;
Kandidaten können einen eigenen, ehrlich benannten Outcome-Tracker führen.
Interviews erhalten Termin, Zeitzone, Teilnehmer, Zusage, Absage,
Verschiebung, ICS und Reminder, getrennt vom Pipeline-Status.
...
**Zweck:** Kritische Journeys werden in Chromium, Firefox und WebKit sowie
mit Tastatur und dokumentierten Assistive-Technology-Smokes bedienbar.
...
**Befunde:** `STH-023`, `STH-025` sowie die UX-/Regressionsebene von
`STH-026`.

**Abhängigkeiten:** die zielrelevanten Phasen aus 20–28. Phase 24 ist bei
...
Locked States. Phase 27 gilt nur bei Multi-Persona-Scope. Capability-, Trust-
und Recruiting-Oberflächen werden in jedem Fall im endgültigen Vertrag
geprüft.
...
**Zweck:** Drei getrennt evidenzierte Tracks vermeiden falsche Prioritäten.
Track 30A liefert früh die kontrollierte Schweizer Berufstaxonomie und
erklärbare Synonym-/Abkürzungs-/Typo-Suche für den tatsächlichen Startcluster.
...
**Befunde:** Track 30A `STH-019` P1 Startcluster-Launch-Gate; Track 30B
`STH-020`, `STH-021`; Track 30C `STH-027` P3/kapazitätsabhängig.
...
vom dokumentierten Trigger ab.

**Pflicht-Gates 30A:** immutable Berufs-Konzepte/Aliase und
...
erfüllen das eingefrorene Recall-/Precision-/p95-Budget. Boost bleibt hinter
Relevanz; Cursor/Alert behandeln Releasewechsel kontrolliert.
...
**Befunde:** `STH-018`, `STH-022`, `STH-028`.
...
**Pflicht-Gates:** vorregistrierte bezahlte KMU-Angebotstests;
...
aus 30A; Package-to-Entitlement-to-Delivery-Matrix; Support-/SLA-Owner;
Enterprise-Integrations-ADR; Salary-LIVE-Gate.
...
```text
19 Baseline
  -> 31A früher Discovery-Track
       (ICP/WTP/Cashflow/Liquidität + Startcluster-Query-/Judgment-Korpus)
  -> 20 Identity/E-Mail/Notifications (technischer Handler/Adaptervertrag)
31A Query-/Judgment-Korpus
  -> 30A P1 Startcluster-Suche/Alert/Recommendation/Cluster V2
  -> kein öffentlicher Clusterlaunch/SEO/Paid Acquisition vor grünem 30A
20
  -> 21 Document/CV Vault (technischer Storage-/Scanvertrag)
20 + 21
  -> 22 Privacy/Legal/Analytics (Domain/Policy, LIVE-Admin noch gesperrt)
20 + 21 + 22
  -> 23 Production Operations/Worker (autonome Aktivierung)
20 + 22 + 23
  -> 25 Admin Security
23 + 31A-Go/No-go
  -> 24 Real Billing/Finance (Sandbox/Domain)
24 + 25 + 31A-Go/No-go
  -> Paid-LIVE-Aktivierung weiterhin gesperrt bis 31B
21 + 25
       -> 26 Company Trust
20 + 25
  -> 27 Multi-Persona Identity
20 + 21 + 22 + 23 + 25 (+ 27, falls im Scope)
  -> 28 Recruiting
20 bis 23 + 25 + 26 + 28 + 30A
  (+ 24 bei Paid Scope, + 27 bei Multi-Persona)
  -> 29 UX/Mobile/Accessibility
23 + 25 + 29
  -> 30B Admin-/Recommendation-Scale
19
  -> 30C Sitemap-Messbaseline
23 + 30C-Baseline
  -> Capacity-Monitoring/Alerts
  -> nur bei Trigger: 30C Sitemap-Index/Shards
22 + 24 + 26 + 28 + 29 + 30A + 30B + 31A
  (+ 30C-Shards nur bei Trigger)
  -> 31B Production-Angebotsfreigabe/Marktevidence
             -> 32 Final Release Audit
```
...
- Nach Phase 19 können das fachliche Design von Phase 21 und das
  Capability-/Persona-Design von Phase 25/27 auf getrennten Branches
  beginnen.
...
- Track 30A beginnt nach Phase 19/31A-Korpus früh. Phase 29 kann die
  Accessibility-/Mobile-Analyse parallel vorbereiten, übernimmt die finale
...
- Phase 25 muss vor Phase 26/27/28 integriert sein; sonst würden Trust-,
  Persona- und Recruiting-Aktionen gegen eine bewegliche
...
- Phase 29 ist ein breiter Portal-/UI-Refactor und beginnt erst nach den
  fachlichen Änderungen 25–28 sowie dem Search-UX-Vertrag aus 30A. Die breiten
  Admin-/Recommendation-Änderungen aus 30B folgen der stabilen UI-/Capability-
  Grundlage. Track 30C berührt Sitemap-Routen nur bei dokumentiertem Trigger.
...
| Startcluster Search P1 | fachlich freigegebenes Golden-/Negativkorpus und versionierte Taxonomie | Search/Alert/Recommendation/Cluster-V2-Parität, Ranking, Cursor, Recall/Precision/p95 | Berufsfachreview je Cluster/Sprache |
...
Der spätere Record `codex-plan/evidence/YYYY-MM-DD-phase-XX.md` enthält
mindestens:
