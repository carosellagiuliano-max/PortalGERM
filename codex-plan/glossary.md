...
| **Lohn-Radar / Salary Radar** | Public salary-orientation tool returning a CHF range from versioned `SalaryBand` rows with source/method/disclaimer. The current dataset is explicitly fictional and Demo-only; without a reviewed LIVE source the Production route returns no values, stays `noindex` and is absent from the sitemap. |
...
| **Mock provider** | Local adapter for an actual external boundary without network calls. It returns deterministic provider behavior; the owning domain persists truthful state (the pure Payment Mock itself writes none). A Mock payment confirms mechanics, never paid conversion, collected revenue or willingness to pay. Analytics/Invoice HTML are internal domain code, not provider mocks. |
| **AVG/AVV Gate** | Fachjuristische/behördliche Einordnung des konkreten Stellenmarkt-, Bewerbungs-, Radar-, Kontakt-, Reveal- und Entgeltflows nach Schweizer Arbeitsvermittlungsgesetz/-verordnung; vor realem Betrieb gegebenenfalls kantonale und bei grenzüberschreitender Vermittlung eidgenössische Bewilligung. |
