...
- [ ] `package.json`-Scripts plattformneutral: `dev`, `build`, `start`, `lint`, `typecheck`, `test`, `test:integration`, `test:e2e` (darf bis 17 als klarer Placeholder exit non-zero/skip policy dokumentieren), `db:generate`, `db:migrate`, `db:seed`, `db:studio`.
...
- [ ] Zod-`envSchema` loads server-only, returns only typed non-secret config and key handles, fails with variable name/reason but never value, and is tested for every invalid/rotation case. A cross-platform `npm run env:generate-local` may generate local secrets without echoing them to logs; generated `.env.local` remains ignored.
