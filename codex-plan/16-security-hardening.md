...
> **PortalGERM status: IMPLEMENTED AND VERIFIED on 23 July 2026.** The immutable code commit is `b052dffe217c0e255664b91ba02c0a39b7321fc5`; see [Phase-16 Evidence](./evidence/2026-07-23-phase-16.md). Security remains layered across all phases; this phase closes the final coverage pass, including separate noindex/no-store controls (ADR-026).
...
- [x] Phase 06 auth + initial headers done
- [x] All feature phases (07–15) complete so every endpoint, SEO/cache path and Boost/Radar flow can be hardened

## Deliverables (checklist)

### Security headers (full set)

- [x] At the installed-version request boundary (`proxy.ts` for Next 16; static headers may remain in `next.config.ts`) set and test per-response headers:
...
- [x] Verify via `curl -I` after a production build + `npm start` (HSTS only takes effect behind real HTTPS termination — note this for the deployment environment)
...
- [x] If using server actions: rely on Next's same-origin enforcement and explicitly check `Origin`/`Referer` for any custom route handlers performing mutations
- [x] Reject cross-origin mutations server-side
- [x] Confirm cookies are `SameSite=Lax`, `Secure` in prod, `HttpOnly`

### IDOR / ownership coverage

- [x] Sweep every route handler/server action that takes an entity id and require a **resource-specific authorized repository/use case** (`findJobForMember`, `findApplicationForCandidate`, `findInvoiceForCompany`, etc.) whose initial query includes tenant/candidate/participant scope. A broad pre-check followed by unscoped lookup is forbidden; missing and foreign objects share the same safe 404.
- [x] Specifically cover:
...
- [x] Test guessed cross-company and nonexistent object UUIDs — both return an indistinguishable safe 404; a route-level role failure without object enumeration may return 403

### Input sanitisation

- [x] All user-generated text rendered to HTML goes through `lib/security/sanitize.ts` (strip-tags or DOMPurify-server) and is rendered as text — never as raw HTML
- [x] Markdown in guide articles (if used) rendered via a safelist (only headings, paragraphs, lists, links with `rel="noopener noreferrer"`)
- [x] Zod enforces type, normalization and length and rejects control/null characters. Plain-text fields may contain angle-bracket text but are always output-escaped (so `<script>` renders inert); approved Markdown is parsed/sanitized through the allowlist. Tests distinguish validation from output encoding.

### File upload restrictions

- [x] CV upload (mock storage): mime whitelist (`application/pdf`, `image/png`, `image/jpeg`, `image/webp`), size ≤ 5 MB, never persist bytes in MVP — metadata only
- [x] Logo/cover in the Mock MVP stores only validated self-hosted/seed asset metadata whose path is under the reviewed `/assets/company-media/` manifest; no arbitrary employer-hosted HTTPS reference, fetch, tracking pixel or fake download exists. This matches `img-src 'self' data:` and `Referrer-Policy`; a real upload/scanner/proxy/CDN policy is a later reviewed release.
- [x] Reject filename traversal patterns

### Rate limiting (final pass)

- [x] Confirmed limits on: login, register, forgot-password, application submit, contact request, lead form, abuse-report submit
- [x] On limit hit: Route Handlers return HTTP `429` plus `Retry-After`; Server Actions return a typed `RATE_LIMITED` domain result with status `429`. Both paths show a friendly German message and write audit action `RATE_LIMITED`.
...
| `CHECKOUT_CREATED` / `ORDER_PAID` / `ORDER_FAILED` / `ORDER_CANCELLED` / `ORDER_EXPIRED` / `INVOICE_ISSUED` / `INVOICE_PAID` / `INVOICE_VOIDED` / `SUBSCRIPTION_ACTIVATED` / `SUBSCRIPTION_CHANGED` / `SUBSCRIPTION_CANCELLING` / `SUBSCRIPTION_EXPIRED` | billing |
...
- [x] User-facing abuse-report dialog reachable from: job detail, company profile, message thread, candidate profile (private), employer applicant card
- [x] Server action creates `AbuseReport` with `targetType`, `targetId`, `reason`, `description`, audit `ABUSE_REPORT_SUBMITTED`
- [x] Verify the Phase 11 Admin queue at `/admin/reports` end to end
- [x] Notification: send mock email `abuse_report_received` to the admin distribution list (template now in the Phase 04 `EmailTemplateKey` union)

### Logging & error handling

- [x] `app/error.tsx` — friendly German error page; never leaks stack traces in production
- [x] `app/not-found.tsx` — friendly 404
- [x] All server paths use one structured logger producing timestamp, level, event name, correlation/trace id, environment and allowlisted entity/reason fields. A redaction layer removes secrets, tokens, raw email/IP, passwords, CV/message/body content and stack details from user responses; `console.*` outside the logger is CI-forbidden.

### Health routes

- [x] `/health/live` reports only process liveness and build identifier; `/health/ready` performs bounded DB/migration readiness and required local dependency checks without leaking connection strings, table contents or provider secrets. Failed readiness returns 503 with a correlation id.

### Privacy headers / preview

- [x] On `/candidate/messages/[threadId]` (Talent Radar threads pre-reveal) set `Cache-Control: no-store` to avoid CDN caching of personal data
- [x] Explicit dynamic/no-store behavior for all personal Employer/Admin routes; `noindex` is a separate SEO instruction and does not satisfy cache protection
- [x] Sensitive out-of-layout routes `/reset-password`, `/invite/[token]`, `/support/[id]`, `/alerts/unsubscribe/[token]`, `/mock/checkout/[orderId]` and local `/dev/mailbox` set `Cache-Control: private, no-store`, `robots: noindex,nofollow`, route-specific `Referrer-Policy: no-referrer`, never include the token/id in Canonical/analytics/log metadata and return generic invalid/foreign responses. `/dev/mailbox` is missing/fail-closed in Production and secret-authenticated in local/CI.

### Final IDOR sweep test plan

- [x] Document a manual test plan in the README: 6 IDOR attempts (one per portal area) using the dev tools network panel and a second user account; expect all to fail.
...
- `lib/providers/email/templates/abuse-report-received.ts` (new template)
...
> **Plan status:** Implemented and verified against code commit `b052dffe217c0e255664b91ba02c0a39b7321fc5`. Automated, PostgreSQL, production-build, HTTP/HSTS and browser checks are recorded in [Phase-16 Evidence](./evidence/2026-07-23-phase-16.md).

- [x] The production build/start HTTP smoke shows CSP+nonce+strict-dynamic, X-Frame-Options DENY, X-Content-Type-Options nosniff, Referrer-Policy and Permissions-Policy; homepage + job detail render with 0 CSP console errors. `npm run test:e2e:hsts` separately proves exact production-only HSTS header emission; local HTTP does not prove TLS/browser enforcement.
- [x] No `Set-Cookie` issues; `session` is `HttpOnly` `Secure` `SameSite=Lax` in prod *(Phase 06; cookie flags unchanged)*
...
| Validation/security | CSP nonce tested with Next hydration/JSON-LD, frame/nosniff/referrer/permissions and production-only HSTS header emission, CSRF/origin, safe redirects, XSS/plain text, upload/import limits, trusted proxy policy, bounded queries. |
...
| Seed | Public demo data stays benign; malicious XSS/XML/JSON, denied CSV, IDOR Tenant A/B, secret/PII canaries and rate cases live in isolated security fixtures. |
