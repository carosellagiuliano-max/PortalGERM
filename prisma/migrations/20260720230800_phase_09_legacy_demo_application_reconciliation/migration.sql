-- Older sealed demo Applications predate the Phase-09 idempotency-key prefix.

BEGIN;
-- Scope reconciliation by both DEMO principals and DEMO jobs instead; LIVE
-- candidate/job evidence remains untouched.
...
WHERE revision."id" = snapshot."jobRevisionId"
  AND snapshot."applicationId" = application."id"
  AND candidate_user."dataProvenance" = 'DEMO'
  AND job."dataProvenance" = 'DEMO';
