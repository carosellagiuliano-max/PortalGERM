...
function InvitationRow({ invite, canManage }: Readonly<{ invite: Team["invitations"][number]; canManage: boolean }>) {
  ...
  return <div className="flex flex-col gap-3 rounded-lg border p-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-medium">{invite.inviteeEmailNormalized}</p><p className="text-xs text-muted-foreground">{invite.intendedRole} · Version {invite.tokenVersion} · gültig bis {formatDateTime(invite.expiresAt)}</p></div>{canManage ? <div className="grid gap-1"><div className="flex gap-2"><form action={resendAction}><input type="hidden" name="invitationId" value={invite.id} /><EmployerSubmitButton pending={resendPending} label="Neu senden" variant="outline" /></form><form action={revokeAction}><input type="hidden" name="invitationId" value={invite.id} /><EmployerSubmitButton pending={revokePending} label="Widerrufen" variant="destructive" /></form></div><EmployerActionFeedback state={resendState.status !== "idle" ? resendState : revokeState} /></div> : null}</div>;
}
